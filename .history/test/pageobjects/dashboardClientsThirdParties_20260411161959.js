...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async waitForClientsThirdPartiesPage() {
        await browser.waitUntil(async () => {
            ...
            const hasSidebar = await this.isVisible(this.clientsSidebarButton);
            return onUrl && (hasTitle || hasSearch || hasCreate || hasGrid || hasNameHeader || hasAddressHeader || hasSidebar);
        }, {
            timeout: 15000,
            timeoutMsg: "Clients / 3rd Parties page did not load expected controls"
        });
    ...
    async openClientsThirdPartiesDirectly() {
        ...
        const directUrl = new URL('/account/clientsParties', currentUrl).toString();
    ...
    async waitForGridOrEmptyState() {
        await browser.waitUntil(async () => {
            ...
            const searchIsVisible = await this.isVisible(this.searchInput);
            const createIsVisible = await this.isVisible(this.createButton);
            const nameHeaderIsVisible = await this.isVisible(this.nameColumnHeader);
            const addressHeaderIsVisible = await this.isVisible(this.addressColumnHeader);
            return gridIsVisible || emptyIsVisible || (searchIsVisible && createIsVisible) || nameHeaderIsVisible || addressHeaderIsVisible;
        }, {
            timeout: 10000,
            timeoutMsg: "Clients / 3rd Parties grid or empty state did not appear"
        }).catch(async () => {
            ...
            if (gridIsVisible || emptyIsVisible || (searchIsVisible && createIsVisible) || nameHeaderIsVisible || addressHeaderIsVisible) {
                return;
            }
