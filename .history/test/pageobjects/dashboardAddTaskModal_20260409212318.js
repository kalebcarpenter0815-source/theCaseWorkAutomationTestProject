...
class DashboardAddTaskModal extends Page {
    ...
    async checkDueByCheckbox() {
        ...
        if (!isChecked) {
            try {
                ...
                const dueByLabel = await this.dueByLabel;

                try {
                    await dueByLabel.click();
                } catch (innerError) {
                    await browser.execute((el) => el.click(), dueByLabel);
                }
            }
    ...
    async selectDateFromPicker(dateString) {
        ...
        const exactDateButton = await $(`button=${dateString}`);
        const hasExactDate = await exactDateButton.isExisting().catch(() => false);

        if (hasExactDate) {
            await exactDateButton.click();
            await browser.pause(300);
            return;
        }

        const fallbackDateButton = await $(`//button[contains(normalize-space(.), "${dateString}") or contains(@aria-label, "${dateString}")]`);
        const hasFallbackDate = await fallbackDateButton.isExisting().catch(() => false);

        if (hasFallbackDate) {
            await fallbackDateButton.click();
            await browser.pause(300);
        }
    }
