...
class DashboardMyCasesPage extends Page {
    ...
    async selectAllAffiliatedParties() {
        const dialog = await $('//div[@role="dialog"][.//*[contains(normalize-space(.), "Add Affiliated Party")]]');
        await dialog.waitForDisplayed({ timeout: 10000 });

        await browser.waitUntil(async () => {
            const counts = await browser.execute((root) => {
                const checkboxCount = root.querySelectorAll('input[type="checkbox"], [role="checkbox"]').length;
                const cardCount = root.querySelectorAll('[class*="Card"], [role="row"]').length;
                return { checkboxCount, cardCount };
            }, dialog);

            return counts.checkboxCount > 0 || counts.cardCount > 0;
        }, {
            timeout: 8000,
            interval: 250,
            timeoutMsg: "Add Affiliated Party options did not render"
        });
        ...
        const scrollDownAndGetState = async () => {
            return await browser.execute((root) => {
                const candidates = [root, ...Array.from(root.querySelectorAll("*"))];
                let target = root;
                let maxDelta = Math.max(0, root.scrollHeight - root.clientHeight);
                ...
                const step = Math.max(140, Math.floor(target.clientHeight * 0.85));
                target.scrollTop = Math.min(target.scrollTop + step, target.scrollHeight);

                const after = target.scrollTop;
                const reachedBottom = after >= target.scrollHeight - target.clientHeight - 2;
                const moved = after > before;

                return { moved, reachedBottom };
            }, dialog);
        };

        const getCardKey = async (card) => {
            return await browser.execute((el) => {
                const normalize = (text) => String(text || "").replace(/\s+/g, " ").trim();
                const cardText = normalize(el ? el.innerText : "");
                const firstLine = cardText.split("\n").map((line) => normalize(line)).filter(Boolean)[0] || "";
                const fallback = el.id || el.getAttribute("data-testid") || "card";
                return normalize(firstLine || fallback);
            }, card);
        };

        const getVisibleCards = async () => {
            const checkboxLike = await dialog.$$('.//input[@type="checkbox"] | .//*[@role="checkbox"]');
            if (checkboxLike.length > 0) {
                return checkboxLike;
            }

            return await dialog.$$('.//*[contains(@class,"fui-Card") or @role="row" or .//input[@type="checkbox"] or .//*[@role="checkbox"]]');
        };

        const isCardSelected = async (card) => {
            const state = await browser.execute((el) => {
                ...
                const checkboxLike = card.querySelector('input[type="checkbox"], [role="checkbox"], [aria-checked]');
                if (checkboxLike) {
                    if (checkboxLike instanceof HTMLInputElement) {
                        return checkboxLike.checked;
                    }
                    const ariaChecked = checkboxLike.getAttribute("aria-checked");
        ...
        // Pass 1: discover unique visible candidates across the full scrollable list.
        ...
        for (let i = 0; i < 80; i += 1) {
            ...
            for (const card of cards) {
                ...
                const key = await getCardKey(card);
                if (key && !candidates.has(key)) {
                    candidates.set(key, true);
                }
            }

            const state = await scrollDownAndGetState();
            if (!state.moved || state.reachedBottom) {
                break;
            }
        }
        ...
        if (!keys.length) {
            throw new Error("No affiliated-party checkboxes were found to select");
        }

        // Randomly pick at least 1 (prefer more when available).
        ...
        const targetKeys = new Set(keys.slice(0, targetCount));

        // Pass 2: scroll again and click selected random targets.
        let selectedCount = 0;
        await resetScrollToTop();
        ...
        for (let i = 0; i < 100 && selectedCount < targetCount; i += 1) {
            ...
            for (const card of cards) {
                ...
                const nowSelected = await isCardSelected(card);
                selectedCount += nowSelected ? 1 : 1;
        ...
        // Hard fallback: guarantee at least one selection.
        if (selectedCount < 1) {
            await resetScrollToTop();
            const cards = await getVisibleCards();
            for (const card of cards) {
        ...
        if (selectedCount < 1) {
            throw new Error(`Could not select affiliated parties. attempted=${targetCount} discovered=${keys.length}`);
        }
