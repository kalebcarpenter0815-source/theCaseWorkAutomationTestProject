...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async openAddPhoneNumberDialog() {
        ...
        await browser.waitUntil(async () => {
            const headingVisible = await this.addPhoneNumberDialogHeading.isDisplayed().catch(() => false);
            const inputVisible = (await this.getVisiblePhoneNumberInput()) !== null;
            return headingVisible || inputVisible;
        }, {
            timeout: 10000,
            timeoutMsg: "Add Phone Number UI did not open"
        });
    }

    async getVisiblePhoneNumberInput() {
        const candidates = await $$('//label[contains(translate(normalize-space(.), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone number")]/following::input[1] | //input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone number")] | //input[contains(translate(@aria-label, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone number")] | //input[contains(translate(@name, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone") and @type="text"] | //input[contains(@data-testid, "phone") and @type="text"]');
    ...
    async addPhoneNumberEntry({ number, type, makePrimary = false }) {
        ...
        const phoneInput = await this.getVisiblePhoneNumberInput();
        if (!phoneInput) {
            throw new Error("No visible Phone Number input was found after opening Add Phone Number");
        }
        await this.clearAndType(phoneInput, number);

        const dropdownVisible = await this.phoneTypeDropdown.isDisplayed().catch(() => false);
        if (dropdownVisible) {
            ...
            if (optionVisible) {
                await option.click();
            } else if (await this.phoneTypeInput.isDisplayed().catch(() => false)) {
                await this.phoneTypeInput.click();
                await this.phoneTypeInput.clearValue();
                await this.phoneTypeInput.setValue(type);
        ...
        const submitVisible = await this.phoneNumberSubmitButton.isDisplayed().catch(() => false);
        if (submitVisible) {
    ...
    async closeAddPhoneNumberDialog() {
        ...
        await browser.waitUntil(async () => {
            const dialogOpen = await this.addPhoneNumberDialogHeading.isDisplayed().catch(() => false);
            const inputVisible = (await this.getVisiblePhoneNumberInput()) !== null;
            return !dialogOpen && !inputVisible;
        }, {
            timeout: 10000,
