...
class DashboardPage extends Page {
    ...
    async ensureEventEditorClosed() {
        const editorIsOpen = await this.eventEditor.isDisplayed().catch(() => false);
    ...
    async getUpcomingEventSummaries() {
        ...
        for (const oneEventCard of visibleEventCards) {
            const titleElement = await oneEventCard.$('.fui-Persona__primaryText');
            const textNodes = await oneEventCard.$$('.fui-Text');
            let titleText = "";
            let dateLabel = "";
            ...
            for (const oneTextNode of textNodes) {
                const rawText = (await oneTextNode.getText()).trim();

                if (/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}$/.test(rawText)) {
                    dateLabel = rawText;
                    break;
                }
            }
    ...
    async assertVisibleEventsWithinDays(maxDays, { allowNoResults = false } = {}) {
        ...
        for (const oneSummary of eventSummaries) {
            if (!oneSummary.parsedDate) {
                throw new Error(`Could not parse an event date from "${oneSummary.dateLabel || "(missing)"}" for "${oneSummary.title || "untitled event"}".`);
            }
