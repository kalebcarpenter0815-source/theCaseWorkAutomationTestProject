...
class DashboardTemplatesPage extends Page {
    ...
    get confirmCopyDialog() {
        return $('//div[@role="dialog" and (.//*[contains(normalize-space(),"Confirm Copy")] or .//*[contains(normalize-space(),"copy this template")])]');
    }

    get confirmDeleteDialog() {
        return $('//div[@role="dialog" and (.//*[contains(normalize-space(),"Confirm Delete")] or .//*[contains(normalize-space(),"delete this template")])]');
    }
    ...
    getRowSelectorByName(templateName) {
        const key = this.toRowSearchKey(templateName).replace(/"/g, '\\"');
        return `//div[@role="row"][.//*[contains(translate(translate(normalize-space(.), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "'’", ""), "${key}")]] | //tr[.//*[contains(translate(translate(normalize-space(.), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "'’", ""), "${key}")]]`;
    }

    getTemplateRowByName(templateName) {
        return $(this.getRowSelectorByName(templateName));
    }

    // Gets the Copy button (1st icon button that appears when you hover a row)
    getCopyButtonInRow(templateName) {
        const rowSelector = this.getRowSelectorByName(templateName);
        return $(`(${rowSelector}//button)[1]`);
    }

    // Gets the Edit button (2nd icon button that appears when you hover a row)
    getEditButtonInRow(templateName) {
        const rowSelector = this.getRowSelectorByName(templateName);
        return $(`(${rowSelector}//button)[2]`);
    }

    // Gets the Delete button (3rd icon button that appears when you hover a row)
    getDeleteButtonInRow(templateName) {
        const rowSelector = this.getRowSelectorByName(templateName);
        return $(`(${rowSelector}//button)[3]`);
    }

    getActionMenuButtonInRow(templateName) {
        const rowSelector = this.getRowSelectorByName(templateName);
        return $(`(${rowSelector}//button)[last()]`);
    }

    getRowMenuAction(actionName) {
        const escaped = actionName.replace(/"/g, '\\"');
        return $(`(//*[@role="menuitem" and (normalize-space()="${escaped}" or .//*[normalize-space()="${escaped}"])])[last()] | (//button[normalize-space()="${escaped}"])[last()] | (//span[normalize-space()="${escaped}"]/ancestor::*[self::button or @role="menuitem"][1])[last()]`);
    }
    ...
    async clickCopyOnRow(templateName) {
        ...
        const copyVisible = await copyBtn.isDisplayed().catch(() => false);

        if (copyVisible) {
            await this.safeClick(copyBtn);
            const copyOpened = await this.confirmCopyDialog.isDisplayed().catch(() => false);
            if (copyOpened) {
                return;
            }
        }

        await this.probeRowButtonsUntil(templateName, async () => this.confirmCopyDialog.isDisplayed().catch(() => false), "Copy");
    }

    async clickEditOnRow(templateName) {
        await this.hoverTemplateRowByName(templateName);
        const editBtn = this.getEditButtonInRow(templateName);
        const editVisible = await editBtn.isDisplayed().catch(() => false);

        if (editVisible) {
            await this.safeClick(editBtn);
            const editOpened = await this.addEditCaseTemplateHeading.isDisplayed().catch(() => false);
            if (editOpened) {
                return;
            }
        }

        await this.probeRowButtonsUntil(templateName, async () => this.addEditCaseTemplateHeading.isDisplayed().catch(() => false), "Edit");
    }

    async clickDeleteOnRow(templateName) {
        await this.hoverTemplateRowByName(templateName);
        const deleteBtn = this.getDeleteButtonInRow(templateName);
        const deleteVisible = await deleteBtn.isDisplayed().catch(() => false);

        if (deleteVisible) {
            await this.safeClick(deleteBtn);
            const deleteOpened = await this.confirmDeleteDialog.isDisplayed().catch(() => false);
            if (deleteOpened) {
                return;
            }
        }

        await this.probeRowButtonsUntil(templateName, async () => this.confirmDeleteDialog.isDisplayed().catch(() => false), "Delete");
    }

    async probeRowButtonsUntil(templateName, successCheck, menuAction) {
        const rowSelector = this.getRowSelectorByName(templateName);

        await this.hoverTemplateRowByName(templateName);
        const rowButtons = await $$(`${rowSelector}//button`);
        ...
        for (let i = 0; i < rowButtons.length; i += 1) {
            ...
            const success = await browser.waitUntil(async () => {
                const ok = await successCheck().catch(() => false);
                return ok;
            }, { timeout: 1200, interval: 150 }).catch(() => false);

            if (success) {
                return;
            }

            const menuVisible = await this.getRowMenuAction(menuAction).isDisplayed().catch(() => false);
            if (menuVisible) {
                await this.safeClick(this.getRowMenuAction(menuAction));
                const successAfterMenu = await browser.waitUntil(async () => {
                    const ok = await successCheck().catch(() => false);
                    return ok;
                }, { timeout: 1500, interval: 150 }).catch(() => false);
        ...
        const menuBtn = this.getActionMenuButtonInRow(templateName);
        const menuBtnVisible = await menuBtn.isDisplayed().catch(() => false);
        if (menuBtnVisible) {
            await this.safeClick(menuBtn);
            await this.safeClick(this.getRowMenuAction(menuAction));
        }
    }
    ...
    async copyTemplateAndConfirm(templateName) {
        ...
        await browser.waitUntil(async () => this.confirmCopyDialog.isDisplayed().catch(() => false), {
            timeout: 10000,
            timeoutMsg: "Confirm Copy dialog did not appear",
        });
    ...
    async deleteTemplateAndConfirm(templateName) {
        ...
        await browser.waitUntil(async () => this.confirmDeleteDialog.isDisplayed().catch(() => false), {
            timeout: 10000,
            timeoutMsg: "Confirm Delete dialog did not appear",
        });
        await this.confirmDialogYes();
        await this.newTemplateButton.waitForDisplayed({ timeout: 10000 });
    }
    ...
    async countTemplatesByExactName(templateName) {
        const selector = this.getRowSelectorByName(templateName);
    ...
    getTemplateRowMenuButtonByName(templateName) {
        return this.getActionMenuButtonInRow(templateName);
    }
    ...
    async openStatusSection(statusName) {
        ...
        const sectionToggle = await $(`(//*[contains(normalize-space(),"Allowed Statuses")]/following::*[self::button or @role="button" or self::div][.//*[normalize-space()="${escaped}"] or normalize-space()="${escaped}"])[1] | //button[normalize-space()="${escaped}"]`);
    ...
    async fillMilestoneAndNestedForms() {
        ...
        const milestoneNameInput = await milestoneContainer.$('.//label[contains(normalize-space(),"Milestone Name")]/following::input[1]');
        const milestoneDescriptionTextarea = await milestoneContainer.$('.//label[normalize-space()="Description"]/following::textarea[1]');
        const milestoneDeliverablesTextarea = await milestoneContainer.$('.//label[normalize-space()="Deliverables"]/following::textarea[1]');

        await this.clearAndType(milestoneNameInput, "New Milestone");
        await this.clearAndType(milestoneDescriptionTextarea, "Who's Deby? YO WIFE!");
        await this.clearAndType(milestoneDeliverablesTextarea, "A present of course!");
        ...
        const eventNameInput = await eventDialog.$('.//label[contains(normalize-space(),"Event Name")]/following::input[1]');
        const eventDaysInput = await eventDialog.$('.//label[contains(normalize-space(),"Days from Created")]/following::input[1]');
        const eventDescriptionTextarea = await eventDialog.$('.//label[normalize-space()="Description"]/following::textarea[1]');

        await this.clearAndType(eventNameInput, "Deby's birthday!");
        await this.clearAndType(eventDaysInput, "12");
        ...
        await this.clearAndType(eventDescriptionTextarea, "This is an IMPORTANT DAY FOR US! ~ Signed, Me");
