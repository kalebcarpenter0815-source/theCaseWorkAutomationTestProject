...
class DashboardPage extends Page {
    ...
    async clickEditButtonFromEvent(event) {
        ...
        for (const selector of candidateSelectors) {
            const candidate = await event.$(selector);
        ...
        for (const selector of ancestorCandidates) {
            const scope = await event.$(selector);
            if (await scope.isExisting()) {
                const editChildren = await scope.$$('button, [role="button"], svg, path');
                for (const child of editChildren) {
                    try {
                        await child.scrollIntoView();
                    } catch (e) {}
                    try {
                        await child.waitForDisplayed({ timeout: 1000 });
                    } catch (e) {}
                    try {
                        await child.click();
                        return;
        ...
        const anyButtons = await event.$$('button, [role="button"]');
        for (const control of anyButtons) {
            try {
                await control.scrollIntoView();
            } catch (e) {}
            try {
                await control.waitForDisplayed({ timeout: 1000 });
            } catch (e) {}
            try {
                await control.click();
                return;
