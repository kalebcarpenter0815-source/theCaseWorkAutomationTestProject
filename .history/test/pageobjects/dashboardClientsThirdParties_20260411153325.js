...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async openAddPhoneNumberDialog() {
        ...
        // Try to find the add phone button - could be in Add Contact dialog or main edit page
        const phoneButtonFound = await browser.execute(() => {
            // Check if we're in an Add Contact dialog
            const contactDialogs = Array.from(
            ...
            if (contactDialogs.length > 0) {
                // We're in the Add Contact dialog - find the phone button within it
                const dialog = contactDialogs[0];
                const phoneButton = dialog.querySelector('[data-testid*="phone"][data-testid*="add"], button[aria-label*="Add"][aria-label*="Phone"], button:has-text("Add Phone")');
                if (phoneButton) {
                    phoneButton.click();
                    return true;
                }
                
                // Fallback: look for any button near "Phone Numbers" text in the dialog
                const phoneLabels = Array.from(dialog.querySelectorAll('*')).filter(el => 
                    el.textContent.includes('Phone Numbers') && el.textContent.length < 50
                );
                if (phoneLabels.length > 0) {
                    const buttons = phoneLabels[0].parentElement?.querySelectorAll('button');
            ...
            // Not in contact dialog - look for button on main page
            const buttons = Array.from(document.querySelectorAll('button'));
            const addPhoneBtn = buttons.find(btn => 
        ...
        if (!phoneButtonFound) {
            ...
            try {
                ...
                await browser.execute((el) => el.click(), await this.addPhoneButton);
            }
        }
    ...
    async ensureCreateNewContactToggle(desiredOn) {
        ...
        const toggled = await browser.execute((shouldBeOn) => {
            ...
            const toggle = labelContainer.querySelector('[role="switch"]') || 
                          ...
                          labelLabel.parentElement?.querySelector('[role="switch"]') ||
