...
class DashboardPage extends Page {
    ...
    async clickEditButtonFromEvent(event) {
        ...
        if (await editButton.isExisting()) {
            await editButton.scrollIntoView({ block: "center", inline: "center" });
        ...
        const clickableChildren = await event.$$("button, [role='button']");
        for (const child of clickableChildren) {
            if (await child.isExisting()) {
                await child.scrollIntoView({ block: "center", inline: "center" });
                await browser.pause(100);
                await browser.execute((el) => el.click(), child);
                return;
            }
        }

        const fallbackButton = await event.$("button");
    ...
    async editAllUpcomingEvents() {
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            if (!event) {
                console.log(`No refreshed event found at index ${i}.`);
                continue;
            }
