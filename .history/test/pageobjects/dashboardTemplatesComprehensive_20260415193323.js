...
class DashboardTemplatesComprehensivePage {
    ...
    async openAndCloseCaseTemplatesInfo() {
        const infoButton = $(
            '//h2[contains(normalize-space(),"Case Templates")]/following::button[1]'
            + ' | //h2[contains(normalize-space(),"Case Templates")]/ancestor::*[self::div][1]//button[contains(@class,"InfoButton") or starts-with(@id,"infolabel-") or @aria-label="information"][1]'
        );

        await infoButton.waitForDisplayed({ timeout: 10000 });
