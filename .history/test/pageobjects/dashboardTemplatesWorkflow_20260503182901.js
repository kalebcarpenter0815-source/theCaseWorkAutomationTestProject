...
class DashboardTemplatesWorkflowPage extends Page {
    ...
    get templatesSearchInput() {
        return $('//*[@data-testid="search-input"] | //input[contains(@placeholder,"Search")]');
    }
    ...
    async searchForTemplateIfPresent(text) {
        const input = this.templatesSearchInput;
    ...
    async clearTemplateSearchIfPresent() {
        await this.searchForTemplateIfPresent("");
    }
    ...
    async hoverTemplateRowByName(templateName) {
        await this.searchForTemplateIfPresent(templateName);
        const row = this.getTemplateRowByName(templateName);
    ...
    async getAllTemplateNames() {
        ...
        await this.clearTemplateSearchIfPresent();
    ...
    async ensureTemplateExists(templateName) {
        return this.ensureTemplateExistsFrom(templateName, templateName.replace(/-Copy$/, ""));
    }

    async ensureTemplateExistsFrom(templateName, sourceTemplateName) {
        const count = await this.countTemplatesByExactName(templateName);
        if (count > 0) {
            return true;
        }

        const sourceExists = await this.isTemplatePresent(sourceTemplateName);
        if (!sourceExists) {
            return false;
        }

        await this.copyTemplateAndConfirm(sourceTemplateName);
        return (await this.countTemplatesByExactName(templateName)) > 0;
    }

    async resolveWorkflowTargets() {
        await this.openCaseTemplatesLanding();

        await browser.waitUntil(async () => {
            const names = await this.getUniqueTemplateNames();
            return names.length > 0;
        }, {
            timeout: 15000,
            interval: 400,
            timeoutMsg: "No template rows found on the Case Templates table.",
        });

        const allNames = await this.getUniqueTemplateNames();

        if (allNames.length === 0) {
            throw new Error("No template rows found on the Case Templates table.");
        }

        const nonCopyNames = allNames.filter((name) => !name.includes("-Copy"));

        const primaryName = allNames.includes("Pikachu")
            ? "Pikachu"
            : (nonCopyNames[0] || allNames[0]);

        const secondaryName = allNames.includes("You're the Best Around")
            ? "You're the Best Around"
            : (nonCopyNames.find((name) => name !== primaryName)
                || allNames.find((name) => name !== primaryName)
                || primaryName);

        const secondaryCopyName = allNames.includes("You're the Best Around-Copy")
    ...
    async ensureBaselineRows(targets = null) {
        const resolvedTargets = targets || await this.resolveWorkflowTargets();

        await this.deleteTemplatesUntilCount(resolvedTargets.primaryName, 1);
        await this.deleteTemplatesUntilCount(resolvedTargets.secondaryName, 1);

        await this.ensureTemplateExistsFrom(resolvedTargets.secondaryCopyName, resolvedTargets.secondaryName);
        await this.deleteTemplatesUntilCount(resolvedTargets.secondaryCopyName, 1);

        const allNames = await this.getAllTemplateNames();
        const nestedCopyNames = [...new Set(allNames.filter((name) => (
            name.startsWith(`${resolvedTargets.primaryName}-Copy`)
            || name.startsWith(`${resolvedTargets.secondaryCopyName}-Copy`)
        )))];

        for (const rowName of nestedCopyNames) {
            const keepCount = rowName === resolvedTargets.secondaryCopyName ? 1 : 0;
            await this.deleteTemplatesUntilCount(rowName, keepCount);
        }
    ...
    async getOrderedTemplateNamesForFullRun(priorityNames = ["Pikachu", "You're the Best Around", "You're the Best Around-Copy"]) {
        await this.openCaseTemplatesLanding();
        const allNames = await this.getUniqueTemplateNames();
    ...
    async getRemainingTemplateNames(priorityNames = ["Pikachu", "You're the Best Around", "You're the Best Around-Copy"]) {
        const orderedNames = await this.getOrderedTemplateNamesForFullRun(priorityNames);
