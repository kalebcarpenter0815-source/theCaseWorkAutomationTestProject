...
class DashboardTemplatesPage extends Page {
    ...
    get addEditFormContainer() {
        return $('//*[contains(normalize-space(),"Add/Edit Case Template")]/ancestor::div[contains(@class,"fui-Card") or contains(@class,"fui-DialogSurface")][1]');
    }
    ...
    async fillCaseTemplateMainFields(data) {
        const form = await this.addEditFormContainer;
        await form.waitForDisplayed({ timeout: 10000 });

        const scopedTemplateNameInput = await form.$('.//label[contains(normalize-space(),"Template Name")]/following::input[1]');
        const scopedTemplateDescriptionInput = await form.$('.//label[contains(normalize-space(),"Template Description")]/following::input[1] | .//label[contains(normalize-space(),"Template Description")]/following::textarea[1]');

        await this.clearAndType(scopedTemplateNameInput, data.templateName);
        await this.clearAndType(scopedTemplateDescriptionInput, data.templateDescription);
