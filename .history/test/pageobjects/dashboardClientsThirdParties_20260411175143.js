...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async openAddPhoneNumberDialog(options = {}) {
        ...
        if (requireAddContactCard) {
            // Do not press Escape in strict Add Contact mode because Escape can close the Add Contact card.
            await this.waitForBackdropToClear(2500);
        } else {
            await this.dismissBlockingBackdrops();
        }
        await browser.pause(200);
        ...
        for (let attempt = 0; attempt < 4; attempt += 1) {
            try {
                if (requireAddContactCard) {
                    await this.waitForBackdropToClear(1500);
                } else {
                    await this.dismissBlockingBackdrops();
                }
                ...
                if (requireAddContactCard) {
                    ...
                    await this.waitForBackdropToClear(1200);
                } else {
                    await browser.keys("Escape").catch(() => null);
                }
                await browser.pause(200);
