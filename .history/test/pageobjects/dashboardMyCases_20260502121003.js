    ...
    async selectAllAffiliatedParties() {
        ...
        const result = await browser.execute(() => {
            ...
            const dialog = Array.from(document.querySelectorAll('[role="dialog"]')).find((node) => {
                return normalize(node.textContent).includes("Add Affiliated Party");
            });

            if (!dialog) {
                return { selectedCount: 0, targetCount: 0, candidateCount: 0, reason: "dialog-not-found" };
            }
            ...
            const getLabelKey = (checkbox) => {
                const card = checkbox.closest('[class*="Card"], [role="row"], div');
                const lines = normalize(card ? card.innerText : "").split("\n").map((line) => normalize(line)).filter(Boolean);
                const firstLine = lines[0] || "";
                const fallback = checkbox.id || checkbox.name || checkbox.getAttribute("data-testid") || "checkbox";
            ...
            const getScrollableContainer = () => {
                ...
                for (const node of candidates) {
                    ...
                    if (canScrollY && score > bestScore + 8) {
                        best = node;
                        bestScore = score;
                    }
                }
            ...
            const collectCandidates = () => {
                ...
                for (let i = 0; i < maxLoops; i += 1) {
                    const visibleCheckboxes = Array.from(dialog.querySelectorAll('input[type="checkbox"]'))
                        .filter((box) => isVisible(box) && isEnabled(box));
                    ...
                    for (const checkbox of visibleCheckboxes) {
                        ...
                        const key = getLabelKey(checkbox);
                        if (key && !map.has(key)) {
                            map.set(key, key);
                        }
                    }
                    ...
                    const reachedBottom = container.scrollTop >= container.scrollHeight - container.clientHeight - 2;

                    if (reachedBottom || container.scrollTop === before) {
                        break;
                    }
                }
            ...
            if (!candidates.length) {
                ...
                return { selectedCount: 0, targetCount: 0, candidateCount: 0, reason: "no-candidates" };
            }

            const minPick = candidates.length > 1 ? 2 : 1;
            const preferredMax = Math.max(minPick, Math.min(candidates.length, Math.max(3, Math.floor(candidates.length * 0.4))));
            const targetCount = Math.floor(Math.random() * (preferredMax - minPick + 1)) + minPick;
            const targetKeys = new Set(shuffle([...candidates]).slice(0, targetCount));
            ...
            for (let i = 0; i < maxLoops && selectedCount < targetCount; i += 1) {
                const visibleCheckboxes = Array.from(dialog.querySelectorAll('input[type="checkbox"]'))
                    .filter((box) => isVisible(box) && isEnabled(box));
            ...
            if (selectedCount === 0) {
                container.scrollTop = 0;
                const firstVisible = Array.from(dialog.querySelectorAll('input[type="checkbox"]'))
                    .find((box) => isVisible(box) && isEnabled(box) && !box.checked);

                if (firstVisible) {
                    firstVisible.click();
                    if (firstVisible.checked) {
                        selectedCount = 1;
                    }
                }
            }
        ...
        if (!result || result.selectedCount < 1) {
            throw new Error(`Could not select affiliated parties. Details: ${JSON.stringify(result)}`);
        }
