...
class DashboardTemplatesPage extends Page {
    ...
    getRowSelectorByName(templateName) {
        ...
        return `//div[@role="row" and .//*[translate(translate(normalize-space(.), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "'’", "")="${key}"]] | //tr[.//*[translate(translate(normalize-space(.), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "'’", "")="${key}"]]`;
    }
    ...
    async clickCopyOnRow(templateName) {
        ...
        await copyBtn.waitForDisplayed({ timeout: 6000 });
        try {
            await copyBtn.click();
        } catch {
            await browser.execute((el) => el.click(), await copyBtn);
        }
    }
    ...
    async clickEditOnRow(templateName) {
        ...
        await editBtn.waitForDisplayed({ timeout: 6000 });
        try {
    ...
    async clickDeleteOnRow(templateName) {
        ...
        await deleteBtn.waitForDisplayed({ timeout: 6000 });
        try {
            await deleteBtn.click();
        } catch {
            await browser.execute((el) => el.click(), await deleteBtn);
        }
    }
    ...
    async copyTemplateAndConfirm(templateName) {
        const newCopyName = `${templateName}-Copy`;
        const beforeCount = await this.countTemplatesByExactName(newCopyName);

        let opened = false;
        for (let i = 0; i < 3; i += 1) {
            await this.clickCopyOnRow(templateName);
            opened = await browser.waitUntil(async () => {
                const dialogOpen = await this.confirmCopyDialog.isDisplayed().catch(() => false);
                const afterCount = await this.countTemplatesByExactName(newCopyName);
                return dialogOpen || afterCount > beforeCount;
            }, {
                timeout: 2500,
                interval: 150,
            }).catch(() => false);
        ...
        if (!opened) {
            throw new Error(`Confirm Copy dialog did not appear for row: ${templateName}`);
        }

        const hasDialog = await this.confirmCopyDialog.isDisplayed().catch(() => false);
        if (hasDialog) {
    ...
    async deleteTemplateAndConfirm(templateName) {
        const beforeCount = await this.countTemplatesByExactName(templateName);

        let opened = false;
        for (let i = 0; i < 3; i += 1) {
            await this.clickDeleteOnRow(templateName);
            opened = await browser.waitUntil(async () => {
                const dialogOpen = await this.confirmDeleteDialog.isDisplayed().catch(() => false);
                const afterCount = await this.countTemplatesByExactName(templateName);
                return dialogOpen || afterCount < beforeCount;
            }, {
                timeout: 2500,
                interval: 150,
        ...
        if (!opened) {
            throw new Error(`Confirm Delete dialog did not appear for row: ${templateName}`);
        }

        const hasDialog = await this.confirmDeleteDialog.isDisplayed().catch(() => false);
        if (hasDialog) {
            await this.confirmDialogYes();
        }
    ...
    async openEditForTemplate(templateName) {
        ...
        throw new Error(`Edit form did not open for row: ${templateName}`);
    }
