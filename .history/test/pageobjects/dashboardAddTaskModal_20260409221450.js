...
class DashboardAddTaskModal extends Page {
    ...
    async selectCaseOption(label) {
        ...
        const optionFound = await browser.waitUntil(async () => {
            const exactOption = await this.getExactCaseOption(label);
            return await exactOption.isDisplayed().catch(() => false);
        }, {
            timeout: 2500,
            interval: 200,
        ...
        let option = await this.getExactCaseOption(label);
        const exactVisible = await option.isDisplayed().catch(() => false);

        if (!exactVisible) {
            option = await this.getCaseOption(label);
        }
    ...
    getExactCaseOption(label) {
        return $(`//*[@role="option" and normalize-space(.) = ${this.escapeXPathString(label)}]`);
    }
