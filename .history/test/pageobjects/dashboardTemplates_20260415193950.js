...
class DashboardTemplatesPage extends Page {
    ...
    async expandAllAllowedStatusGroups() {
        ...
        // Expand every collapsed status group (New/Active/Completed/Closed/Removed and any future groups).
    ...
    async getVisibleAllowedStatusCheckboxes() {
        ...
        const all = [...roleBoxes, ...nativeBoxes];
        const visible = [];
        for (const checkbox of all) {
            const isVisible = await checkbox.isDisplayed().catch(() => false);
            if (isVisible) {
    ...
    async ensureAllAllowedStatusesCheckedWithRetries(maxPasses = 4) {
        ...
        for (let pass = 0; pass < maxPasses; pass += 1) {
            ...
            // Verify all currently visible checkboxes are selected.
            await this.expandAllAllowedStatusGroups();
            const verifyBoxes = await this.getVisibleAllowedStatusCheckboxes();
            const allChecked = (await Promise.all(
                verifyBoxes.map(async (checkbox) => this.isCheckboxSelected(checkbox))
            )).every(Boolean);
            ...
            if (!clickedAny) {
                // Nothing changed in this pass and still not fully checked.
                continue;
            }
        }
