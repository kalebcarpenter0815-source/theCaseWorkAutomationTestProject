...
class DashboardMyCasesPage extends Page {
    ...
    get searchClearButton() {
        return $('//input[@data-testid="search-input"]/following::button[.//*[name()="path" and contains(@d,"m2.59 2.72")]][1]');
    }

    get dashboardSidebarButton() {
        return $('//button[@data-testid="vert-nav-dashboard"] | //a[@data-testid="vert-nav-dashboard"] | //button[contains(normalize-space(.), "Dashboard")] | //a[contains(normalize-space(.), "Dashboard")]');
    }
    ...
    get applyTemplateButton() {
        return $('//label[contains(normalize-space(.), "Apply Template")]/following::button[@aria-haspopup="menu"][1] | //button[@data-testid="apply-template-button"] | //button[@aria-haspopup="menu" and (contains(normalize-space(.), "Template") or contains(@aria-label, "Template"))]');
    }

    get templateOptions() {
        return $$('//div[@role="option"] | //li[@role="option"] | //button[@role="menuitem"] | //span[contains(@class, "MenuItem__content")]');
    }
    ...
    getTemplateOption(templateName) {
        const safeText = this.escapeXpathText(templateName);
        return $(`//*[(@role="option" or @role="menuitem" or self::span or self::div) and (normalize-space(.)=${safeText} or .//*[normalize-space(.)=${safeText}] or contains(normalize-space(.), ${safeText}))]`);
    }
    ...
    get caseTypeCombobox() {
        return $('//input[@data-testid="case-type-combobox"] | //label[contains(normalize-space(.), "Case Type")]/following::input[1] | //*[@role="combobox" and (contains(@aria-label, "Case Type") or contains(@name, "caseType"))]');
    }
    ...
    get partyCombobox() {
        return $('//input[@data-testid="party-combobox"] | //label[contains(normalize-space(.), "Retained By")]/following::input[1] | //input[contains(@aria-label, "Retained By")]');
    }
    ...
    // Status Dropdown
    get statusCombobox() {
        return $('//input[@data-testid="case-status-combobox"] | //label[contains(normalize-space(.), "Status")]/following::input[1] | //*[@role="combobox" and (contains(@aria-label, "Status") or contains(@name, "status"))]');
    }

    get retainedByInfoButton() {
        return $('//div[contains(@class,"fui-Field")][.//input[@data-testid="party-combobox"]]//button[@aria-label="information"] | //label[contains(normalize-space(.), "Retained By")]/following::button[contains(@aria-label, "information") or contains(@aria-label, "info")][1] | //label[contains(normalize-space(.), "Retained By")]/ancestor::*[self::div or self::fieldset][1]//button[contains(@aria-label, "information") or contains(@aria-label, "info")]');
    }

    get statusInfoButton() {
        return $('//div[contains(@class,"fui-Field")][.//input[@data-testid="case-status-combobox"]]//button[@aria-label="information"] | //label[contains(normalize-space(.), "Status")]/following::button[contains(@aria-label, "information") or contains(@aria-label, "info")][1] | //label[contains(normalize-space(.), "Status")]/ancestor::*[self::div or self::fieldset][1]//button[contains(@aria-label, "information") or contains(@aria-label, "info")]');
    }
    ...
    get editEventButton() {
        return $('//button[@data-testid="event-edit-button"] | //button[contains(@aria-label, "Edit Event") or contains(@title, "Edit Event")] | //button[contains(@aria-label, "Edit") and ancestor::*[contains(normalize-space(.), "Events")]] | //*[name()="path" and contains(@d,"M17.18 2.93")]/ancestor::button[1]');
    }

    get deleteEventButton() {
        return $('//button[@data-testid="event-delete-button"] | //button[contains(@aria-label, "Delete Event") or contains(@title, "Delete Event")] | //button[contains(@aria-label, "Delete") and ancestor::*[contains(normalize-space(.), "Events")]] | //*[name()="path" and contains(@d,"M8.5 4h3")]/ancestor::button[1]');
    }
    ...
    async selectComboboxValue(element, value) {
        ...
        const exists = await input.waitForExist({ timeout: 3000 }).then(() => true).catch(() => false);
        if (!exists) {
            return false;
        }
        ...
        await input.clearValue().catch(() => null);
        await input.setValue(value);
        await browser.pause(250);

        const safeValue = value.trim().toLowerCase();
        const clickedOption = await browser.execute((targetText) => {
            ...
            const options = Array.from(document.querySelectorAll('[role="option"], li[role="option"], [role="listbox"] *'))
                .filter((el) => visible(el))
                .map((el) => ({ el, text: normalize(el.textContent) }));

            const exact = options.find((option) => option.text === targetText);
            if (exact) {
                exact.el.click();
                return true;
            }

            const partial = options.find((option) => option.text.includes(targetText));
            if (partial) {
                partial.el.click();
                return true;
            }
    ...
    async clearSearchInputAfterEntry() {
        ...
        const clearButtonVisible = await this.searchClearButton.isDisplayed().catch(() => false);

        if (clearButtonVisible) {
            await this.clickSearchClearButton();
            return;
        }
    ...
    async clickDashboardSidebarButton() {
        const dashboardButton = await this.dashboardSidebarButton;
        await dashboardButton.waitForDisplayed({ timeout: 10000 });
        await this.scrollIntoView(dashboardButton);
        await this.clickElement(dashboardButton);
    }

    async isDashboardSidebarSelected() {
        const dashboardButton = await this.dashboardSidebarButton;
        const ariaCurrent = await dashboardButton.getAttribute("aria-current");

        if (ariaCurrent && ariaCurrent !== "false") {
            return true;
        }

        const className = (await dashboardButton.getAttribute("class")) || "";
        if (/active|selected|current/i.test(className)) {
            return true;
        }
        ...
        const myCasesVisible = await this.myCasesSection.isDisplayed().catch(() => false);
        const searchVisible = await this.searchInput.isDisplayed().catch(() => false);
        return myCasesVisible || searchVisible;
    }
    ...
    async selectTemplate(templateName) {
        ...
        const buttonVisible = await button.isDisplayed().catch(() => false);
        if (!buttonVisible) {
            return;
        }
    ...
    async clickCalendarDayCell(day) {
        const targetPoint = await browser.execute((dayText) => {
            ...
            if (!grids.length) {
                return { clicked: false, reason: 'no-grids' };
            }
            ...
            if (!leftGrid) {
                return { clicked: false, reason: 'no-left-grid' };
            }

            const matches = Array.from(leftGrid.querySelectorAll('*'))
                .filter(isVisible)
                .filter((element) => (element.textContent || '').trim() === dayText)
                .map((element) => {
                    const gridcell = element.closest('[role="gridcell"]');
                    const focusable = element.closest('[tabindex]');
                    const button = element.closest('button');
                    const cell = gridcell || focusable || button || element;
                ...
                .filter((item) => item.text === dayText && item.disabled !== 'true')
                .sort((left, right) => left.rect.top - right.rect.top || left.rect.left - right.rect.left);

            if (!matches.length) {
                return { clicked: false, reason: 'no-matches' };
            }
        ...
        if (!targetPoint?.found) {
            throw new Error(`Could not click day cell "${day}" in the left calendar grid (${targetPoint?.reason || 'unknown'})`);
        }

        await browser.action('pointer', { parameters: { pointerType: 'mouse' } })
            .move({ x: targetPoint.x, y: targetPoint.y, origin: 'viewport' })
            .down({ button: 0 })
            .up({ button: 0 })
            .perform();
    ...
    async getRetainedDateSnapshot() {
        return await browser.execute(() => {
            const field = document.querySelector('div.fui-Field:has(input[name="retainedDate"])');
            const inputs = Array.from(document.querySelectorAll('input[name="retainedDate"], div.fui-Field input[placeholder*="date" i]'));
            ...
            return `${field?.textContent || ''} ${inputValues}`.replace(/\s+/g, ' ').trim();
        });
    }
    ...
    async forceRetainedDateValue(month, day) {
        ...
        const displayDate = new Date(2026, monthIndex, Number(day)).toDateString();
        const isoDate = `2026-${String(monthIndex + 1).padStart(2, '0')}-${String(Number(day)).padStart(2, '0')}`;

        await browser.execute((displayValue, isoValue) => {
            ...
            const namedInputs = Array.from(document.querySelectorAll('input[name="retainedDate"]'));
            namedInputs.forEach((input) => setInputValue(input, displayValue));

            const visibleDateInputs = Array.from(document.querySelectorAll('div.fui-Field input[placeholder*="date" i], div.fui-Field input'))
                .filter((input) => {
            ...
            visibleDateInputs.forEach((input) => {
                if ((input.getAttribute('name') || '') === 'retainedDate' || (input.getAttribute('placeholder') || '').toLowerCase().includes('date')) {
                    setInputValue(input, displayValue);
                    input.setAttribute('data-retained-date-iso', isoValue);
                }
            });
        }, displayDate, isoDate);
    }
    ...
    async selectRetainedDate(month, day) {
        ...
        await browser.waitUntil(async () => {
            ...
            updated = snapshot.includes(month) && snapshot.includes(day);
            return updated;
        }, {
            timeout: 1500,
    ...
    async selectAllAffiliatedParties() {
        const selectionResult = await browser.execute(() => {
            const normalize = (text) => String(text || "").replace(/\s+/g, " ").trim();

            const dialog = Array.from(document.querySelectorAll('[role="dialog"]')).find((node) => {
                const text = normalize(node.textContent);
                const hasHeading = text.includes("Add Affiliated Party");
                const hasCancel = Array.from(node.querySelectorAll('button')).some((btn) => normalize(btn.textContent) === "Cancel");
                const hasSubmit = Array.from(node.querySelectorAll('button')).some((btn) => normalize(btn.textContent).includes("Add Affiliated Party"));
                const hasAddParty = Array.from(node.querySelectorAll('button')).some((btn) => normalize(btn.textContent).includes("Add Party"));
                return hasHeading && hasCancel && hasSubmit && hasAddParty;
            });

            if (!dialog) {
                return { selectedCount: 0, reason: "dialog-not-found" };
            }
            ...
            const getScrollContainer = () => {
                ...
                let best = dialog;
                let bestDelta = Math.max(0, dialog.scrollHeight - dialog.clientHeight);
                ...
                for (const node of candidates) {
                    ...
                    const delta = Math.max(0, node.scrollHeight - node.clientHeight);
                    if (canScrollY && delta > bestDelta) {
                        best = node;
                        bestDelta = delta;
                    }
                }
            ...
            const getToggleCandidates = () => {
                const footerSubmit = Array.from(dialog.querySelectorAll('button')).find((btn) =>
                    normalize(btn.textContent).includes("Add Affiliated Party")
                );
                const footerTop = footerSubmit ? footerSubmit.getBoundingClientRect().top : Number.POSITIVE_INFINITY;

                const buttonCandidates = Array.from(dialog.querySelectorAll('button')).filter((btn) => {
                    ...
                    const testId = normalize(btn.getAttribute('data-testid')).toLowerCase();

                    if (text.includes('add party') || text.includes('cancel') || text.includes('add affiliated party') || text.includes('create')) {
                        return false;
                    }

                    if (aria.includes('add party') || aria.includes('cancel') || aria.includes('add affiliated party') || testId.includes('add-party')) {
                        return false;
                    }

                    const rect = btn.getBoundingClientRect();
                    if (rect.top >= footerTop - 6) {
                        return false;
                    }

                    // Tile selector controls are usually icon-only near the right side of each card.
                    return text.length === 0 || rect.width <= 36;
                });

                const checkboxCandidates = Array.from(dialog.querySelectorAll('input[type="checkbox"], [role="checkbox"]'))
                    .filter((el) => isVisible(el));

                return checkboxCandidates.length > 0 ? checkboxCandidates : buttonCandidates;
            };

            const getKeyForNode = (node) => {
                const card = node.closest('[class*="Card"], [role="row"], div');
                const text = normalize(card ? card.innerText : node.getAttribute('aria-label') || node.textContent || '');
                const firstLine = text.split("\n").map((line) => normalize(line)).filter(Boolean)[0] || "";
                return firstLine || normalize(node.id || node.getAttribute('data-testid') || node.getAttribute('aria-label') || 'option');
            };
            ...
            for (let i = 0; i < 80; i += 1) {
                for (const node of getToggleCandidates()) {
                ...
                const step = Math.max(140, Math.floor(scrollContainer.clientHeight * 0.85));
                ...
                const reachedBottom = after >= scrollContainer.scrollHeight - scrollContainer.clientHeight - 2;
                if (after === before || reachedBottom) {
                    break;
                }
            }

            const keys = Array.from(discovered.keys());
            if (!keys.length) {
                scrollContainer.scrollTop = originalTop;
                return { selectedCount: 0, reason: "no-candidates" };
            }

            const minPick = keys.length > 1 ? 2 : 1;
            const maxPick = Math.max(minPick, Math.min(keys.length, Math.max(3, Math.floor(keys.length * 0.35))));
            const targetCount = Math.floor(Math.random() * (maxPick - minPick + 1)) + minPick;
            ...
            const target = new Set(keys.slice(0, targetCount));
            let selectedCount = 0;
            scrollContainer.scrollTop = 0;

            for (let i = 0; i < 100 && selectedCount < targetCount; i += 1) {
                for (const node of getToggleCandidates()) {
                    const key = getKeyForNode(node);
                    if (!target.has(key)) {
                        continue;
                    ...
                    (node instanceof HTMLElement ? node : node.closest('button, [role="checkbox"], input[type="checkbox"]'))?.click();
                    selectedCount += 1;
                    target.delete(key);
            ...
            if (selectedCount < 1) {
                scrollContainer.scrollTop = 0;
                const first = getToggleCandidates()[0];
                if (first) {
                    (first instanceof HTMLElement ? first : first.closest('button, [role="checkbox"], input[type="checkbox"]'))?.click();
                    selectedCount = 1;
                }
            }

            scrollContainer.scrollTop = originalTop;
            return { selectedCount, reason: "ok" };
        });

        if (!selectionResult || selectionResult.selectedCount < 1) {
            throw new Error(`Could not select affiliated parties. ${JSON.stringify(selectionResult)}`);
        }
    ...
    async clickEditEvent() {
        ...
        const editButton = await this.editEventButton;
        const exists = await editButton.isExisting().catch(() => false);
        if (!exists) {
            return;
        }

        await this.scrollIntoView(editButton);
    ...
    async clickDeleteEvent() {
        ...
        const deleteButton = await this.deleteEventButton;
        const exists = await deleteButton.isExisting().catch(() => false);
        if (!exists) {
            return;
        }

        await this.scrollIntoView(deleteButton);
