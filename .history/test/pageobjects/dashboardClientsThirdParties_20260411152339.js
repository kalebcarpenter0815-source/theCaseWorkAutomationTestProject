...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async addPhoneNumberEntry({ number, type, makePrimary = false }) {
        ...
        const dialogContainer = await this.getAddPhoneDialogContainer();
        const dialogInputs = await this.getPhoneInputsFromDialog();

        if (dialogInputs.length === 0) {
            await this.addPhoneNumberDialogHeading.click();
            await browser.keys(number);
        } else {
            const phoneInput = dialogInputs[0];
            await this.clearAndType(phoneInput, number);
        }

        const phoneTypeDropdownInDialog = await dialogContainer.$('.//label[contains(normalize-space(.), "Phone type")]/following::button[1] | .//button[contains(normalize-space(.), "Select or Enter Phone Type")]');
        const dropdownVisible = await phoneTypeDropdownInDialog.isDisplayed().catch(() => false);
        if (dropdownVisible) {
            await phoneTypeDropdownInDialog.click();
            const option = await $(`//*[(@role="option" or self::span or self::div) and normalize-space(.)="${type}"]`);
            const optionVisible = await option.waitForDisplayed({ timeout: 2000 }).then(() => true).catch(() => false);

            if (optionVisible) {
                await option.click();
            } else if (dialogInputs.length > 1) {
                await this.clearAndType(dialogInputs[1], type);
        ...
        if (makePrimary) {
            const primaryToggleInDialog = await dialogContainer.$('.//*[@role="switch" or @type="checkbox"][contains(@aria-label, "Primary") or contains(@id, "primary") or contains(@name, "primary")] | .//label[contains(normalize-space(.), "Primary")]/following::*[@role="switch" or @type="checkbox"][1]');
            const primaryVisible = await primaryToggleInDialog.isDisplayed().catch(() => false);
            if (primaryVisible) {
                await primaryToggleInDialog.click();
            }
        }

        const submitInDialog = await dialogContainer.$('.//button[normalize-space(.)="Submit" or normalize-space(.)="Create" or normalize-space(.)="Add" or @type="submit"]');
        const submitVisible = await submitInDialog.isDisplayed().catch(() => false);
        if (submitVisible) {
    ...
    async fillAddContactForm(data) {
        // First ensure we're on the Add Contact dialog
        await this.addContactHeading.waitForDisplayed({ timeout: 10000 });
        
        // Get contact form inputs using JavaScript-based discovery
        const contactInputs = await browser.execute(() => {
            const dialog = Array.from(
                document.querySelectorAll('[role="dialog"], [class*="Dialog"], [class*="Popover"], [class*="Card"], .fui-DialogSurface')
            ).find(el => el.textContent.includes('Add Contact'));
            
            if (!dialog) return {};
            
            // Get all inputs in the dialog
            const inputs = Array.from(dialog.querySelectorAll('input[type="text"]:not([disabled]):not([readonly])'));
            
            // Find inputs by their relative position or attributes
            return {
                all: inputs.map((inp, idx) => ({
                    index: idx,
        ...
        // Use the found inputs methodically
        if (contactInputs.all && contactInputs.all.length > 0) {
            // Try to map inputs by their attributes
            const inputMap = {};
            for (let i = 0; i < contactInputs.all.length; i++) {
                const inp = contactInputs.all[i];
                const text = (inp.placeholder + inp.name + inp.label + inp.testId).toLowerCase();
                if (text.includes('email')) inputMap.email = i;
                if (text.includes('name')) inputMap.name = i;
                if (text.includes('address') && !text.includes('address 2')) inputMap.address1 = i;
                if (text.includes('address 2')) inputMap.address2 = i;
                if (text.includes('city')) inputMap.city = i;
                if (text.includes('state')) inputMap.state = i;
                if (text.includes('zip')) inputMap.zip = i;
                if (text.includes('title')) inputMap.title = i;
            }

            // Use mapped inputs with fallback to selectors
            let nameInput = null;
            try {
                if (inputMap.name !== undefined) {
                    const inputs = await this.addContactHeading.$$(
                        'ancestor::*[contains(@class, "Dialog") or contains(@class, "Popover") or @role="dialog"][1]//input[type="text"]:not([disabled]):not([readonly])'
                    );
                    if (inputs[inputMap.name]) {
                        nameInput = inputs[inputMap.name];
                    }
                }
            } catch (e) {}
            
            if (!nameInput) {
                nameInput = this.addContactNameInput;
            }
            await this.clearAndType(nameInput, data.name);
        } else {
            // Fallback to original selectors
            await this.clearAndType(this.addContactNameInput, data.name);
        }
        
        await this.clearAndTypeIfVisible(this.addContactTitleInput, data.title);
        
        // For email, try JavaScript navigation
        try {
            const emailEntered = await browser.execute((email) => {
                const dialog = Array.from(
                    document.querySelectorAll('[role="dialog"], [class*="Dialog"], [class*="Popover"], [class*="Card"], .fui-DialogSurface')
                ).find(el => el.textContent.includes('Add Contact'));
                ...
                for (const input of inputs) {
                    if (input.placeholder.toLowerCase().includes('email') || 
                        ...
                        input.getAttribute('data-testid')?.toLowerCase().includes('email')) {
                        input.focus();
                        input.value = email;
        ...
        if (data.address2) {
            await this.clearAndTypeIfVisible(this.addContactAddress2Input, data.address2);
        }
