...
async clickEditButtonFromEvent(event) {
        // wait for hover to reveal edit button, try different selectors
        await browser.pause(2000);
        
        // try general edit SVG first
        let editButton = await event.$('svg.fui-Icon');
        if (await editButton.isExisting()) {
            await editButton.scrollIntoView();
            await editButton.waitForClickable({ timeout: 5000 });
            await editButton.click();
            console.log('Clicked general edit SVG button');
            return;
        }

        // fallback to original data-testid style
        ...
        if (!(await editButton.isExisting())) {
            throw new Error('Edit button was not found for this event');
        }
