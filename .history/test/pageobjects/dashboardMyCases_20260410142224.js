...
class DashboardMyCasesPage extends Page {
    ...
    async closeOpenDialogs() {
        for (let attempt = 0; attempt < 6; attempt++) {
            ...
            const deleteNoButton = await this.confirmDeleteNoButton;
            if (await deleteNoButton.isDisplayed().catch(() => false)) {
                await this.clickElement(deleteNoButton);
                await browser.pause(250);
                closedSomething = true;
            }

            const eventCloseButton = await $('//div[@role="dialog" and .//*[contains(normalize-space(.), "Event")]]//button[@data-testid="event-cancel-button" or normalize-space()="Close"]');
            if (await eventCloseButton.isDisplayed().catch(() => false)) {
                await this.clickElement(eventCloseButton);
                await browser.pause(250);
                closedSomething = true;
            }

            const affiliatedPartyCancel = await $('//div[@role="dialog" and .//*[contains(normalize-space(.), "Add Affiliated Party")]]//button[@data-testid="affiliated-party-dialog-cancel-button" or contains(normalize-space(.), "Cancel")]');
            if (await affiliatedPartyCancel.isDisplayed().catch(() => false)) {
                await this.clickElement(affiliatedPartyCancel);
                await browser.pause(250);
                closedSomething = true;
            }

            const addContactCancel = await $('//div[@role="dialog" and .//*[contains(normalize-space(.), "Add Contact")]]//button[contains(normalize-space(.), "Cancel")]');
            if (await addContactCancel.isDisplayed().catch(() => false)) {
                await this.clickElement(addContactCancel);
                await browser.pause(250);
                closedSomething = true;
            }
