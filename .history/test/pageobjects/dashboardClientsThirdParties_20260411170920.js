...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async fillAddContactForm(data) {
        ...
        }, { timeout: 8000, interval: 300, timeoutMsg: "Add Contact Name field did not appear" }).catch(() => null);
        ...
        const nameOk = await this.fillDialogInput(dialog, "Name", data.name, [dialog.$('.//input[contains(@placeholder, "Name")]')]);
        const titleOk = await this.fillDialogInput(dialog, "Title", data.title, [dialog.$('.//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "title")]')]);
        const emailOk = await this.fillDialogInput(dialog, "Email", data.email, [dialog.$('.//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "email")]')]);
        const addressOk = await this.fillDialogInput(dialog, "Address", data.address1, [dialog.$('.//input[@placeholder="Enter Address"]')]);
        const address2Ok = await this.fillDialogInput(dialog, "Address 2", data.address2, [dialog.$('.//input[@placeholder="Enter Address 2"]')]);
        const cityOk = await this.fillDialogInput(dialog, "City", data.city, [dialog.$('.//input[@placeholder="Enter City"]')]);
        const stateOk = await this.fillDialogInput(dialog, "State", data.state, [dialog.$('.//input[@placeholder="Enter State"]')]);
        const zipOk = await this.fillDialogInput(dialog, "Zip", data.zip, [dialog.$('.//input[@placeholder="Enter Zip"]')]);

        if (!nameOk || !titleOk || !emailOk || !addressOk || !address2Ok || !cityOk || !stateOk || !zipOk) {
            throw new Error("Add Contact form did not fill every required text field");
        }
    }
    ...
    async ensureCreateNewContactToggle(desiredOn) {
        ...
        if (isOn !== desiredOn) {
            ...
            }, { timeout: 8000, interval: 300, timeoutMsg: "Add Contact form did not render after toggling" }).catch(() => null);
        }
