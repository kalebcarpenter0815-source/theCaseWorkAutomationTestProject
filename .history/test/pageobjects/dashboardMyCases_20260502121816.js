...
class DashboardMyCasesPage extends Page {
    ...
    async selectAllAffiliatedParties() {
        await this.affiliatedPartyDialog.waitForDisplayed({ timeout: 10000 });

        const dialog = await this.affiliatedPartyDialog;
        ...
        const getCheckboxKey = async (checkbox) => {
            return await browser.execute((el) => {
                const normalize = (text) => String(text || "").replace(/\s+/g, " ").trim();
                const card = el.closest('[class*="Card"], [role="row"], label, div');
                const cardText = normalize(card ? card.innerText : "");
                const firstLine = cardText.split("\n").map((line) => normalize(line)).filter(Boolean)[0] || "";
                const fallback = el.id || el.name || el.getAttribute("data-testid") || "checkbox";
                return normalize(firstLine || fallback);
            }, checkbox);
        };

        const getSelectableCheckboxes = async () => {
            return await dialog.$$('.//input[@type="checkbox" and not(@disabled)] | .//*[@role="checkbox" and @aria-checked]');
        };
        ...
        const isChecked = async (checkbox) => {
            ...
            return await checkbox.isSelected().catch(() => false);
        };
        ...
        for (let i = 0; i < 80; i += 1) {
            const checkboxes = await getSelectableCheckboxes();
            ...
            for (const checkbox of checkboxes) {
                ...
                const checked = await isChecked(checkbox);
                if (checked) {
                    continue;
                }

                const key = await getCheckboxKey(checkbox);
        ...
        for (let i = 0; i < 100 && selectedCount < targetCount; i += 1) {
            ...
            for (const checkbox of checkboxes) {
                ...
                const nowChecked = await isChecked(checkbox);
                if (nowChecked) {
                    selectedCount += 1;
        ...
        if (selectedCount < 1) {
            ...
            for (const checkbox of checkboxes) {
                ...
                if (!checked) {
                    ...
                    await this.clickElement(checkbox);
                    await browser.pause(100);
                    selectedCount = (await isChecked(checkbox)) ? 1 : 0;
                    break;
                }
            }
        }
