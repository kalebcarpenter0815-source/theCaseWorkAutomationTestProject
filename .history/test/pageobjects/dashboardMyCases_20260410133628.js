...
class DashboardMyCasesPage extends Page {
    ...
    get addContactCancelButton() {
        return $('//button[contains(normalize-space(.), "Cancel")]');
    }
    ...
    get estimatedHoursInput() {
        return $('input[data-testid*="estimated"], input[placeholder*="estimated hours"], input[placeholder*="Estimated hours"]');
    }
    ...
    async selectPartyFromDropdown(partyName) {
        ...
        await input.setValue(partyName);
        await browser.pause(300);

        const option = await $(`//span[normalize-space()="${partyName}"] | //div[normalize-space()="${partyName}"] | //button[normalize-space()="${partyName}"]`);
        await this.clickElement(option);
    }
