...
class DashboardPage extends Page {
    ...
    get notesButton() {
        return $('[data-testid="event-notes-button"], button=Notes, [role="tab"]=Notes');
    }
...
async areEventsWithinDays(maxDays) {
    ...
    async getVisibleEditButtons() {
        const editButtons = await $$('button[aria-label="Edit"], [data-testid^="case-event-edit"], [data-testid*="event-edit"]');

        const visibleButtons = [];

        for (const oneButton of editButtons) {
    ...
    async openNotesPanel() {
        const notesButton = await this.notesButton;
