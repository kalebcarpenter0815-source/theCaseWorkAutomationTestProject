...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async getPhoneInputsFromDialog() {
        const scopedInputs = await $$(
            '(//div[@role="dialog"][.//*[contains(normalize-space(.), "Add Phone Number")]] | //div[contains(@class, "Dialog")][.//*[contains(normalize-space(.), "Add Phone Number")]] | //div[contains(@class, "Popover")][.//*[contains(normalize-space(.), "Add Phone Number")]] | //div[contains(@class, "Card")][.//*[contains(normalize-space(.), "Add Phone Number")]])//input[@type="text" and not(@disabled) and not(@readonly)]'
        );

        const fallbackInputs = await $$(
            '//input[@type="text" and not(@disabled) and not(@readonly) and (contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone") or contains(translate(@name, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone") or contains(translate(@data-testid, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone"))]'
        );

        const allTextInputs = scopedInputs.length > 0 ? scopedInputs : fallbackInputs;
        ...
        for (const input of allTextInputs) {
            ...
            const testId = ((await input.getAttribute("data-testid")) || "").toLowerCase();

            const looksLikeAddress = nameAttr.includes("address") || placeholder.includes("address") || testId.includes("address");
            if (!looksLikeAddress) {
                filtered.push(input);
            }
        }
