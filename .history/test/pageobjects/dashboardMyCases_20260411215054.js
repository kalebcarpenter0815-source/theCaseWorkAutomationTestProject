...
class DashboardMyCasesPage extends Page {
    ...
    async selectRetainedDate(month, day) {
        ...
        const monthButton = await rightGrid.$(`.//button[normalize-space()="${month}"] | .//*[@role="gridcell" and normalize-space()="${month}"]`);
        await monthButton.waitForDisplayed({ timeout: 5000 });
        ...
        await browser.waitUntil(async () => {
            ...
            const dayCell = await leftGrid.$(`.//*[@role="gridcell" and normalize-space()="${day}"] | .//button[normalize-space()="${day}" or .//*[normalize-space()="${day}"]]`);
            return await dayCell.isExisting().catch(() => false);
        ...
        await browser.execute((gridElement, dayText) => {
            const normalize = (value) => (value || '').replace(/\s+/g, ' ').trim();

            const targetCell = Array.from(gridElement.querySelectorAll('[role="gridcell"], button'))
                .find((element) => normalize(element.textContent) === dayText && element.getAttribute('aria-disabled') !== 'true');

            if (!targetCell) {
                return false;
            }

            const clickable = targetCell.querySelector('button, [tabindex]') || targetCell;
