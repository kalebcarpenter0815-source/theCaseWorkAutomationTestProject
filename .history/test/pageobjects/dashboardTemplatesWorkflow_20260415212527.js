...
class DashboardTemplatesWorkflowPage extends Page {
    ...
    getTemplateRowByName(templateName) {
        ...
        return $(`
            //tr[
                .//td[1][normalize-space()=${safeName} or .//*[normalize-space()=${safeName}]]
            ]
            |
            //div[@role="row"][
                .//*[@role="gridcell"][1][normalize-space()=${safeName} or .//*[normalize-space()=${safeName}]]
            ]
        `);
    }
    ...
    async clickMenuAction(actionLabel) {
        ...
        const menuAction = $(`
            //*[@role="menuitem" and (
                normalize-space()=${this.escapeXPathText(actionLabel)}
                or translate(@aria-label, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')=${safeAction}
                or .//*[normalize-space()=${this.escapeXPathText(actionLabel)}]
            )]
            |
            //button[
                normalize-space()=${this.escapeXPathText(actionLabel)}
    ...
    async performCopyDeleteSequence() {
        ...
        const allAfterCopy = await this.getAllTemplateNames();
        const copiedRows = [...new Set(allAfterCopy.filter((name) => name.includes("-Copy")))];
        for (const copiedName of copiedRows) {
            if (copiedName === "You're the Best Around-Copy") {
                const keepOneCount = await this.countTemplatesByExactName(copiedName);
                while (keepOneCount > 1) {
                    await this.deleteTemplateWithDeclineThenConfirm(copiedName);
                    const updatedCount = await this.countTemplatesByExactName(copiedName);
                    if (updatedCount <= 1) {
                        break;
                    }
                }
            ...
            const copiedCount = await this.countTemplatesByExactName(copiedName);
            for (let i = 0; i < copiedCount; i += 1) {
                await this.deleteTemplateWithDeclineThenConfirm(copiedName);
            }
        }
