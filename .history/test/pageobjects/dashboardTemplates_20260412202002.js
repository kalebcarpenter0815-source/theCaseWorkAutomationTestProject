...
class DashboardTemplatesPage extends Page {
    ...
    getTemplateRowByName(templateName) {
        const smartName = templateName.replace(/'/g, "’");
        return $(`//tr[.//*[normalize-space()="${templateName}" or normalize-space()="${smartName}" or contains(normalize-space(),"${templateName.replace("You're the ", "Best Around")}")]] | //div[@role="row"][.//*[normalize-space()="${templateName}" or normalize-space()="${smartName}" or contains(normalize-space(),"${templateName.replace("You're the ", "Best Around")}")]]`);
    }

    // Gets the Copy button (1st icon button that appears when you hover a row)
    getCopyButtonInRow(templateName) {
        const smartName = templateName.replace(/'/g, "’");
        return $(`(//div[@role="row"][.//*[normalize-space()="${templateName}" or normalize-space()="${smartName}"]]//button)[1]`);
    }

    // Gets the Edit button (2nd icon button that appears when you hover a row)
    getEditButtonInRow(templateName) {
        const smartName = templateName.replace(/'/g, "’");
        return $(`(//div[@role="row"][.//*[normalize-space()="${templateName}" or normalize-space()="${smartName}"]]//button)[2]`);
    }

    // Gets the Delete button (3rd icon button that appears when you hover a row)
    getDeleteButtonInRow(templateName) {
        const smartName = templateName.replace(/'/g, "’");
        return $(`(//div[@role="row"][.//*[normalize-space()="${templateName}" or normalize-space()="${smartName}"]]//button)[3]`);
    }
    ...
    async countTemplatesByExactName(templateName) {
        const smartName = templateName.replace(/'/g, "’");
        const selector = `//div[@role="row"][.//*[normalize-space()="${templateName}" or normalize-space()="${smartName}"]]`;
