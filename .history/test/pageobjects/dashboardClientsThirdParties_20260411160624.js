...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    get searchInput() {
        return $('//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "search clients/3rd parties") or contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "search clients") or contains(translate(@aria-label, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "search")]');
    }
    ...
    async getDialog(titleText) {
        const dialog = $(`(//h2[contains(normalize-space(.), "${titleText}")] | //h3[contains(normalize-space(.), "${titleText}")])/ancestor::*[@role="dialog" or contains(@class, "Dialog") or contains(@class, "Card") or contains(@class, "fui-DialogSurface")][1]`);
    ...
    async isOnClientsThirdPartiesPage() {
        ...
        const goodPage = hasTitle || hasSearch || hasCreate || hasGrid || hasNameHeader;
    ...
    async searchForClient(searchText) {
        await this.typeInto(this.searchInput, searchText);
        await browser.pause(350);
    }
    ...
    async addPhoneNumberEntry({ number, type, makePrimary = false }) {
        ...
        const phoneNumberWasTyped = await browser.execute((dialogElement, phoneNumber) => {
            ...
            for (const input of textInputs) {
                ...
                const looksLikePhoneNumber = allText.includes('phone number') || (allText.includes('phone') && !allText.includes('type'));
        ...
        if (!phoneNumberWasTyped) {
            const phoneInput = await this.getVisibleElement([
                dialog.$('.//label[contains(normalize-space(.), "Phone Number")]/following::input[1]'),
                dialog.$('.//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone number")]'),
                dialog.$('.//input[contains(translate(@name, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "phone") and not(contains(translate(@name, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "type"))]')
            ]);
        ...
        if (typeButton) {
            ...
            await browser.pause(300);
            const option = this.phoneTypeOption(type);
            if (await option.isDisplayed().catch(() => false)) {
            ...
            if (typeInput) {
                await this.typeInto(typeInput, type);
                await browser.keys("Enter");
            } else {
                await browser.keys(type);
                await browser.keys("Enter");
    ...
    async fillAddContactForm(data) {
        ...
        await this.fillDialogInput(dialog, "Name", data.name, [dialog.$('.//input[contains(@placeholder, "Name")]')]);
        await this.fillDialogInput(dialog, "Title", data.title, [dialog.$('.//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "title")]')]);
        await this.fillDialogInput(dialog, "Email", data.email, [dialog.$('.//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "email")]')]);
        await this.fillDialogInput(dialog, "Address", data.address1, [dialog.$('.//input[@placeholder="Enter Address"]')]);
        await this.fillDialogInput(dialog, "Address 2", data.address2, [dialog.$('.//input[@placeholder="Enter Address 2"]')]);
        await this.fillDialogInput(dialog, "City", data.city, [dialog.$('.//input[@placeholder="Enter City"]')]);
        await this.fillDialogInput(dialog, "State", data.state, [dialog.$('.//input[@placeholder="Enter State"]')]);
        await this.fillDialogInput(dialog, "Zip", data.zip, [dialog.$('.//input[@placeholder="Enter Zip"]')]);
    }
    ...
    async ensureCreateNewContactToggle(desiredOn) {
        ...
        const isOn = ariaValue === "true" || checkedValue;
