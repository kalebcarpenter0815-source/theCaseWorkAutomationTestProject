...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async getPhoneInputsFromDialog(container) {
        const allTextInputs = await container.$$(
    ...
    async addPhoneNumberEntry({ number, type, makePrimary = false }) {
        ...
        const dialogInputs = await this.getPhoneInputsFromDialog(dialogContainer);

        if (dialogInputs.length === 0) {
            throw new Error("No usable text inputs found inside Add Phone Number popup");
        }
