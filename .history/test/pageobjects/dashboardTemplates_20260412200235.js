...
class DashboardTemplatesPage extends Page {
    ...
    async fillMilestoneAndNestedForms() {
        ...
        await this.waitForDialogWithText("Milestone");
        const milestoneDialog = await this.getTopVisibleDialog();

        await this.clearAndType(this.getFieldByLabelInContainer(milestoneDialog, "Milestone Name"), "New Milestone");
        await this.clearAndType(this.getFieldByLabelInContainer(milestoneDialog, "Description"), "Who's Deby? YO WIFE!");
        await this.clearAndType(this.getFieldByLabelInContainer(milestoneDialog, "Deliverables"), "A present of course!");

        const addEventInMilestone = await milestoneDialog.$('.//button[contains(normalize-space(),"Add Event")]');
        ...
        const milestoneDialogAfterSaveEvent = await this.getTopVisibleDialog();
        const addEventInMilestoneAgain = await milestoneDialogAfterSaveEvent.$('.//button[contains(normalize-space(),"Add Event")]');
        ...
        const milestoneDialogAfterEventCancel = await this.getTopVisibleDialog();
        const addTaskInMilestone = await milestoneDialogAfterEventCancel.$('.//button[contains(normalize-space(),"Add Task")]');
        ...
        await this.clearAndType(this.getFieldByLabelInContainer(taskDialog, "Task to complete"), "I need to buy a gift for Deby.");
        ...
        const milestoneDialogAfterSaveTask = await this.getTopVisibleDialog();
        const addTaskInMilestoneAgain = await milestoneDialogAfterSaveTask.$('.//button[contains(normalize-space(),"Add Task")]');
        ...
        const milestoneDialogForSubmit = await this.getTopVisibleDialog();
        const submitMilestoneButton = await milestoneDialogForSubmit.$('.//button[normalize-space()="Submit" or normalize-space()="Save"]');
        await this.safeClick(submitMilestoneButton);
        await this.waitForDialogToClose("Milestone");

        await this.safeClick(this.addMilestoneButton);
        await this.waitForDialogWithText("Milestone");
        const milestoneDialogForCancel = await this.getTopVisibleDialog();
        const cancelMilestoneButton = await milestoneDialogForCancel.$('.//button[normalize-space()="Cancel"]');
