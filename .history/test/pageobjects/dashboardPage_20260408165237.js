...
async clickEditButtonFromEvent(event) {
        ...
        // longer wait for hover reveal
        await browser.pause(3000);
        
        // try SVG edit icon specifically
        let editButton = await event.$('svg.fui-Icon:has(path[d^="M17.18 2.93"])');
        if (!(await editButton.isExisting())) {
            editButton = await event.$('svg.fui-Icon path[d*="M17.18"]');
        }
        if (!(await editButton.isExisting())) {
            editButton = await event.$('svg[aria-hidden="true"]');
        }
        ...
        editButton = await event.$('[data-testid^="case-event-edit"]');
        if (!(await editButton.isExisting())) {
            const eventId = await event.getAttribute('data-testid');
            if (eventId) {
                editButton = await $(`[data-testid="case-event-edit-${eventId.replace('event-case-persona-', '')}"]`);
            }
        }

        if (!(await editButton.isExisting())) {
            // super fallback - any button/SVG in hovered area
            editButton = await event.$('svg, button');
            if (await editButton.isExisting()) {
                console.log('Using super fallback selector');
            } else {
                throw new Error('No edit button found with any selector');
            }
        }
        ...
        await editButton.click();
        console.log('Clicked edit button with fallback');
    }
