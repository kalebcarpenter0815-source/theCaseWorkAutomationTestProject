...
class DashboardPage extends Page {
    ...
    async clickEditButtonFromEvent(event) {
        let editButton = await event.$('[data-testid^="case-event-edit-"]');
    ...
    async editFirstEventFullFlow() {
        ...
        await browser.action('pointer')
            .move({ origin: reopenedEvent })
            .perform();

        const editButtonAgain = await reopenedEvent.$('[data-testid^="case-event-edit-"]');
        await editButtonAgain.waitForDisplayed({ timeout: 5000 });
