...
class DashboardClientsThirdPartiesPage extends Page {
    get clientsSidebarButton() {
        return $('//button[@data-testid="vert-nav-clients-parties" or @data-testid="vert-nav-clients/parties" or @data-testid="vert-nav-clientsparties"] | //a[contains(normalize-space(.), "Clients / 3rd Parties") or contains(normalize-space(.), "Clients/3rd Parties") or contains(normalize-space(.), "Clients/Parties")] | //button[contains(normalize-space(.), "Clients / 3rd Parties") or contains(normalize-space(.), "Clients/3rd Parties") or contains(normalize-space(.), "Clients/Parties")]');
    }
    ...
    async getAddContactCardContainer() {
        const candidates = [
            $('//div[@role="dialog"][.//h2[contains(normalize-space(.), "Add Contact")] or .//h3[contains(normalize-space(.), "Add Contact")]]'),
            $('//div[contains(@class, "fui-DialogSurface")][.//h2[contains(normalize-space(.), "Add Contact")] or .//h3[contains(normalize-space(.), "Add Contact")]]'),
            $('//div[contains(@class, "fui-DialogBody")][.//h2[contains(normalize-space(.), "Add Contact")] or .//h3[contains(normalize-space(.), "Add Contact")]]'),
            $('//div[contains(@class, "fui-Dialog")][.//h2[contains(normalize-space(.), "Add Contact")] or .//h3[contains(normalize-space(.), "Add Contact")]]')
        ];
    ...
    async waitForBackdropToClear(timeoutMs = 3000) {
        await browser.waitUntil(async () => {
            return !(await this.dialogBackdrop.isDisplayed().catch(() => false));
        }, {
            timeout: timeoutMs,
            interval: 150,
            timeoutMsg: "Dialog backdrop did not clear in time"
        }).catch(() => null);
    }

    async waitForClientsSidebarButton() {
        const sidebarVisible = await this.clientsSidebarButton.waitForDisplayed({ timeout: 5000 }).then(() => true).catch(() => false);
        if (sidebarVisible) {
            return;
        }

        const currentUrl = await browser.getUrl();
        const directClientsUrl = new URL('/account/clientsparties', currentUrl).toString();
        await browser.url(directClientsUrl);
    ...
    async openClientsThirdPartiesDirectly() {
        ...
        const loadedDirectly = await this.waitForClientsThirdPartiesPage().then(() => true).catch(() => false);
        if (loadedDirectly) {
            return;
        }
        ...
        const sidebarVisible = await this.clientsSidebarButton.isDisplayed().catch(() => false);
        if (sidebarVisible) {
            await this.clickElement(this.clientsSidebarButton);
            await this.waitForClientsThirdPartiesPage();
        }
    ...
    async clearSearchUsingXButton() {
        if (!(await this.isOnClientsThirdPartiesPage())) {
            await this.openClientsThirdPartiesFromSidebar();
            await this.waitForGridOrEmptyState();
        }

        const clearButton = await this.getVisibleElement([
            $('(//input[@type="search"]/ancestor::*[self::div or self::span][1]//button[contains(translate(@aria-label, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "clear") or contains(translate(@title, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "clear") or normalize-space(.)="X" or normalize-space(.)="x"])[1]'),
            $('((//label[contains(normalize-space(.), "Search")] | //span[contains(normalize-space(.), "Search")])[1]/following::button[contains(translate(@aria-label, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "clear") or normalize-space(.)="X" or normalize-space(.)="x"])[1]'),
            $('//button[contains(translate(@aria-label, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "clear search") or contains(translate(@title, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "clear search")]')
        ]);
        ...
        if (clearButton) {
            try {
                ...
                await browser.execute((el) => el.click(), clearButton).catch(() => null);
            }
            await browser.pause(250);
        }

        const input = await this.getVisibleElement([
            this.searchInput,
            $('input[placeholder*="Search Clients"], input[placeholder*="Clients/3rd Parties"], input[aria-label*="Search Clients"], input[type="search"]'),
            $('//label[contains(normalize-space(.), "Search")]/following::input[1]')
        ]);
    ...
    async getClientNamesFromGrid(limit = 8) {
        const maxItems = Math.max(1, limit);

        const domNames = await browser.execute((max) => {
            const unique = [];
            ...
            for (const row of rows) {
                ...
                const parts = String(firstCell.innerText || "")
                    .split(/\r?\n/)
                    .map((value) => value.trim())
                    .filter(Boolean)
                    .filter((value) => !/^[A-Z]{1,2}$/.test(value));
        ...
        if (Array.isArray(domNames) && domNames.length > 0) {
            return domNames;
        }
        ...
        for (const row of rows) {
            ...
            const nameCell = await row.$('.//*[@role="gridcell"][1] | ./td[1]');
            if (!(await nameCell.isExisting().catch(() => false))) {
                continue;
            }

            const raw = await nameCell.getText().catch(() => "");
    ...
    async openAddPhoneNumberDialog(options = {}) {
        const { requireAddContactCard = false } = options;
        if (!requireAddContactCard) {
            await this.dismissBlockingBackdrops();
        }
        await browser.pause(200);

        const alreadyOpen = await this.addPhoneNumberDialogHeading.isDisplayed().catch(() => false);
        if (alreadyOpen) {
            return;
        }

        const addContactDialog = await this.getAddContactCardContainer();
        let button = null;

        if (requireAddContactCard) {
            const addContactOpen = await this.addContactHeading.isDisplayed().catch(() => false);
            if (!addContactOpen) {
                throw new Error("Add Contact card is not open while trying to use its Phone Numbers button");
            }

            button = await this.getVisibleElement([
                // Strict Add Contact card scope anchored by the Add Contact heading and Phone Numbers section.
                $('((//h2[contains(normalize-space(.), "Add Contact")] | //h3[contains(normalize-space(.), "Add Contact")])/ancestor::*[.//*[contains(normalize-space(.), "Phone Numbers")] and .//button[normalize-space(.)="Cancel"]][1]//label[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"])[1]'),
                $('((//h2[contains(normalize-space(.), "Add Contact")] | //h3[contains(normalize-space(.), "Add Contact")])/ancestor::*[.//*[contains(normalize-space(.), "Phone Numbers")] and .//button[normalize-space(.)="Cancel"]][1]//*[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"])[1]'),
                $('((//*[contains(normalize-space(.), "Create New Contact")]/ancestor::*[.//*[contains(normalize-space(.), "Phone Numbers")] and .//button[normalize-space(.)="Cancel"]][1])//*[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"])[1]')
            ]);

            if (!button) {
                await this.ensureCreateNewContactToggle(true).catch(() => null);
                button = await this.getVisibleElement([
                    $('((//*[contains(normalize-space(.), "Create New Contact")]/ancestor::*[.//*[contains(normalize-space(.), "Phone Numbers")] and .//button[normalize-space(.)="Cancel"]][1])//*[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"])[1]'),
                    $('((//h2[contains(normalize-space(.), "Add Contact")] | //h3[contains(normalize-space(.), "Add Contact")])/ancestor::*[.//*[contains(normalize-space(.), "Phone Numbers")] and .//button[normalize-space(.)="Cancel"]][1]//*[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"])[1]')
                ]);
        ...
        if (!button && addContactDialog) {
            button = await this.getVisibleElement([
                // Exact icon button under the "Phone Numbers" label in Add Contact card
                addContactDialog.$('.//label[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"][1]'),
                addContactDialog.$('.//*[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"][1]'),
        ...
        if (!button && !requireAddContactCard) {
            button = await this.getVisibleElement([
        ...
        if (!button) {
            if (requireAddContactCard) {
                throw new Error("Add Contact card Phone Numbers button was not available");
            }
            throw new Error("Phone add button not available in current context");
        }

        // Backdrop flashes can intercept this click; retry after dismissing overlays.
        for (let attempt = 0; attempt < 4; attempt += 1) {
            try {
                if (requireAddContactCard) {
                ...
                if (requireAddContactCard) {
                    const stillInAddContact = await this.addContactHeading.isDisplayed().catch(() => false);
                    if (!stillInAddContact) {
                        throw new Error("Add Contact card closed while trying to click its Phone Numbers button");
                    }
                } else {
                    await browser.keys("Escape").catch(() => null);
                }
    ...
    async addPhoneNumberEntry({ number, type, makePrimary = false }, options = {}) {
        try {
            await this.openAddPhoneNumberDialog(options);
        } catch (error) {
            if (options.requireAddContactCard) {
                throw error;
            }
            return;
        }
        ...
        if (!dialog) {
            if (options.requireAddContactCard) {
                throw new Error("Add Phone Number dialog did not open inside Add Contact card");
            }
            return;
        }
        ...
        const closedAfterSubmit = await browser.waitUntil(async () => {
        ...
        if (!closedAfterSubmit) {
            // Retry one final submit path before failing.
            await browser.keys("Enter").catch(() => null);
            await browser.waitUntil(async () => {
                return !(await this.addPhoneNumberDialogHeading.isDisplayed().catch(() => false));
            }, {
                timeout: 4000,
                interval: 250,
                timeoutMsg: "Add Phone Number dialog still open after submit retry"
            });
        }
    }
    ...
    async verifyAddContactDialogReadyForPhoneNumbers() {
        ...
        const addPhoneInDialog = await this.getVisibleElement([
            dialog.$('.//label[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"][1]'),
            dialog.$('.//*[contains(normalize-space(.), "Phone Numbers")]/following::button[@data-testid="phone-panel-add-button"][1]'),
    ...
    async ensureCreateNewContactToggle(desiredOn) {
        ...
        const isCreateFormVisible = async () => {
            const nameField = await this.getVisibleElement([
                dialog.$('.//input[contains(@placeholder, "name of the Contact")]'),
                dialog.$('.//label[contains(normalize-space(.), "Name")]/following::input[1]')
            ]);
        ...
        const isSelectContactsVisible = async () => {
        ...
        // If the target state is already visible, do not block on finding/toggling the switch.
        if (desiredOn && await isCreateFormVisible()) {
            return;
        }
        if (!desiredOn && await isSelectContactsVisible()) {
            return;
        }

        let visibleDataTestIdToggle = null;

        await browser.waitUntil(async () => {
            const allDataTestIdToggles = await $$('//*[@data-testid="select-contacts-create-new-contact-link"]');
            visibleDataTestIdToggle = await this.getVisibleElement(allDataTestIdToggles);
            return Boolean(visibleDataTestIdToggle);
        }, {
            timeout: 5000,
            interval: 250,
            timeoutMsg: "Create New Contact toggle did not appear in Add Contact dialog"
        }).catch(() => null);

        const toggle = visibleDataTestIdToggle || await this.getVisibleElement([
            ...
            dialog.$('.//label[contains(normalize-space(.), "Create New Contact")]/preceding::input[@type="checkbox"][1]'),
            dialog.$('.//label[contains(normalize-space(.), "Create New Contact")]/following::input[@type="checkbox"][1]'),
            this.createNewContactToggle
        ]);

        if (!toggle) {
            const createNewContactLabel = await this.getVisibleElement([
                dialog.$('.//label[contains(normalize-space(.), "Create New Contact")]'),
                $('//label[contains(normalize-space(.), "Create New Contact")]')
            ]);

            if (!createNewContactLabel) {
                throw new Error("Create New Contact toggle was not found in Add Contact dialog");
            }

            await this.clickElement(createNewContactLabel);

            if (desiredOn) {
                await browser.waitUntil(async () => await isCreateFormVisible(), {
                    timeout: 8000,
                    interval: 300,
                    timeoutMsg: "Add Contact form did not render after clicking Create New Contact label"
                });
            } else {
                await browser.waitUntil(async () => await isSelectContactsVisible(), {
                    timeout: 8000,
                    interval: 300,
                    timeoutMsg: "Select Contacts view did not render after clicking Create New Contact label"
                });
            }
