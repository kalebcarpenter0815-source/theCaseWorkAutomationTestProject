...
async areEventsWithinDays(maxDays) {
    ...
    async findFirstEditableEventByTitle(titleText) {
        ...
        for (let i = 0; i < allEvents.length; i++) {
            ...
            if (!titleValue.includes(titleText)) {
                continue;
            }
    ...
    async runRequestedCaseTitleFlow(titleToFind, updatedEventText, updatedDescriptionText, noteText) {
        ...
        if (!selectedCase) {
            console.warn(`I could not find an editable case titled ${titleToFind}.`);
            return false;
        }
        ...
        if (!reopenedCase) {
            console.warn(`I saved ${titleToFind}, but could not reopen it for note verification.`);
            return false;
        }
