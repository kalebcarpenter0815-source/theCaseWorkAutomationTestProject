...
class DashboardClientsThirdPartiesPage extends Page {
    ...
    async verifyAddContactDialogReadyForPhoneNumbers() {
        ...
        if (!dialog) {
            throw new Error("Add Contact dialog is not open");
        }
        ...
        const createNewContactToggle = await this.getVisibleElement([
        ...
        const hasCoreIdentityFields = Boolean(nameInput || titleInput || emailInput);
        const hasContactModeControl = Boolean(createNewContactToggle);

        if (!addPhoneInDialog || !hasCoreIdentityFields || !hasContactModeControl) {
            throw new Error("Add Contact dialog did not match expected layout before second phone number input");
        }
    }
