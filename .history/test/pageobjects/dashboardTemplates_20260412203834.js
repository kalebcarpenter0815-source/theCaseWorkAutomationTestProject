...
class DashboardTemplatesPage extends Page {
    ...
    async copyTemplateAndConfirm(templateName) {
        ...
        for (let i = 0; i < 3; i += 1) {
            ...
            opened = await browser.waitUntil(async () => this.confirmCopyDialog.isDisplayed().catch(() => false), {
                timeout: 2500,
                interval: 150,
            }).catch(() => false);
            if (opened) {
    ...
    async deleteTemplateAndConfirm(templateName) {
        ...
        for (let i = 0; i < 3; i += 1) {
            ...
            opened = await browser.waitUntil(async () => this.confirmDeleteDialog.isDisplayed().catch(() => false), {
                timeout: 2500,
                interval: 150,
            }).catch(() => false);
            if (opened) {
