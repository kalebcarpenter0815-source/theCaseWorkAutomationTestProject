...
async areEventsWithinDays(maxDays) {
    ...
    async findFirstEditableTestEvent() {
        ...
        for (let i = 0; i < allEvents.length; i++) {
            ...
            const editButton = await this.getEditButtonFromEventCard(oneEvent);
            const buttonIsVisible = await editButton.isDisplayed().catch(() => false);

            if (await editButton.isExisting() && buttonIsVisible) {
                return { event: oneEvent, editButton };
            }
        }
    ...
    async runRequestedTestMay2QuagmireFlow() {
        const testEventThing = await this.findFirstEditableTestEvent();
        ...
        const reopenedTestEventThing = await this.findFirstEditableTestEvent();
