...
const DASHBOARD_EMPTY_STATE_XPATH = '//*[contains(normalize-space(.), "No upcoming events") or contains(normalize-space(.), "No results") or contains(normalize-space(.), "No data")]';
...
class DashboardPage extends Page {
    ...
    get eventDialogBackdrop() {
        return $('div.fui-DialogSurface__backdrop');
    }
    ...
    async maybeVisualPause(label, durationMs = 1200) {
        if (process.env.VISUAL_DEMO === "1") {
            console.log(`[VISUAL DEMO] ${label}`);
            await browser.pause(durationMs);
        }
    }

    async ensureEventEditorClosed() {
        const isEventEditorVisible = async () => this.eventEditor.isDisplayed().catch(() => false);
        const isBackdropVisible = async () => this.eventDialogBackdrop.isDisplayed().catch(() => false);

        if (!(await isEventEditorVisible()) && !(await isBackdropVisible())) {
            return;
        }
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
            ...
            if (await closeButton.isDisplayed().catch(() => false)) {
                try {
                    await closeButton.waitForClickable({ timeout: 2000 });
                    await closeButton.click();
                } catch (error) {
                    await browser.execute((el) => el.click(), closeButton).catch(() => false);
                }
            } else if (await this.cancelEventButton.isDisplayed().catch(() => false)) {
                try {
                    await this.cancelEventButton.waitForClickable({ timeout: 2000 });
                    await this.cancelEventButton.click();
                } catch (error) {
                    await browser.execute((el) => el.click(), await this.cancelEventButton).catch(() => false);
                }
            } else {
                await browser.keys("Escape").catch(() => false);
            }

            await browser.waitUntil(async () => {
                return !(await isEventEditorVisible()) && !(await isBackdropVisible());
            }, {
                timeout: 5000,
                interval: 150,
                timeoutMsg: "The event editor stayed open after trying to close it."
            }).catch(() => false);
        }
        ...
        await browser.waitUntil(async () => {
            ...
            timeout: 5000,
            interval: 150,
            timeoutMsg: "The event editor or its backdrop did not fully close."
        });

        await this.waitForDashboardResultsToSettle();
    ...
    get emptyStateBanner() {
        return $(DASHBOARD_EMPTY_STATE_XPATH);
    }
    ...
    async getFilterText() {
        ...
        const valueAttribute = await this.filterDropdown.getAttribute("value");

        if (valueAttribute) {
            return valueAttribute;
        }

        return (await this.filterDropdown.getText()).trim();
    }
    ...
    async findFilterOptionByText(text) {
        const option = await $(`//*[@role="option" or @role="menuitem" or @role="menuitemradio" or @role="menuitemcheckbox" or self::button][contains(normalize-space(.), "${text}")]`);
        return option;
    }
    ...
    async selectOptionByText(text) {
        ...
        const option = await this.findFilterOptionByText(text);
    ...
    async waitForDashboardResultsToSettle() {
        await browser.pause(600);
    }

    normalizeDashboardDate(dateValue) {
    ...
    parseDashboardDateLabel(labelText) {
        if (!labelText) {
            return null;
        }

        const match = labelText.trim().match(/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2})$/);
        ...
        const normalizedToday = this.normalizeDashboardDate(currentDate);

        const candidateThisYear = new Date(currentYear, monthIndex, dayNumber);
        const normalizedThisYear = this.normalizeDashboardDate(candidateThisYear);

        if (normalizedThisYear >= normalizedToday) {
            return normalizedThisYear;
        }

        return this.normalizeDashboardDate(new Date(currentYear + 1, monthIndex, dayNumber));
    }

    getDaysFromToday(targetDate) {
        const normalizedToday = this.normalizeDashboardDate(new Date());
        const normalizedTargetDate = this.normalizeDashboardDate(targetDate);

        if (!normalizedTargetDate) {
            return null;
        }

        return Math.floor((normalizedTargetDate.getTime() - normalizedToday.getTime()) / (1000 * 60 * 60 * 24));
    }

    async getVisibleUpcomingEventCards() {
        const allEventCards = await this.eventCards;
        const visibleEventCards = [];

        for (const oneEventCard of allEventCards) {
            if (await oneEventCard.isDisplayed().catch(() => false)) {
                visibleEventCards.push(oneEventCard);
            }
        }

        return visibleEventCards;
    }

    async countVisibleUpcomingEvents() {
        const visibleEventCards = await this.getVisibleUpcomingEventCards();
        return visibleEventCards.length;
    }

    async getUpcomingEventSummaries() {
        const visibleEventCards = await this.getVisibleUpcomingEventCards();
        const summaries = [];

        for (const oneEventCard of visibleEventCards) {
            const cardRoot = await this.getEventCardRoot(oneEventCard);
            const titleElement = await cardRoot.$('.fui-Persona__primaryText');
            let titleText = "";
            let dateLabel = "";

            if (await titleElement.isExisting().catch(() => false)) {
                titleText = (await titleElement.getText()).trim();
            }

            const combinedCardText = (await cardRoot.getText()).replace(/\s+/g, " ").trim();
            const dateMatch = combinedCardText.match(/\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2})\b/);
            ...
            summaries.push({
                title: titleText,
                dateLabel,
                parsedDate: this.parseDashboardDateLabel(dateLabel)
            });
        }
    ...
    async hasUpcomingEventEmptyState() {
        return this.emptyStateBanner.isDisplayed().catch(() => false);
    }

    async assertVisibleEventsWithinDays(maxDays, { allowNoResults = false } = {}) {
        const eventSummaries = await this.getUpcomingEventSummaries();

        if (eventSummaries.length === 0) {
            if (allowNoResults) {
                return {
                    eventCount: 0,
                    noResults: await this.hasUpcomingEventEmptyState(),
                    skippedDateChecks: 0,
                    validatedDateChecks: 0
                };
            }

            throw new Error(`Expected visible dashboard events for a ${maxDays}-day filter, but none were shown.`);
        }

        let validatedDateChecks = 0;
        let skippedDateChecks = 0;

        for (const oneSummary of eventSummaries) {
            if (!oneSummary.parsedDate) {
                skippedDateChecks += 1;
                continue;
            }

            const daysFromToday = this.getDaysFromToday(oneSummary.parsedDate);

            if (daysFromToday < 0 || daysFromToday > maxDays) {
                throw new Error(`Event "${oneSummary.title || "untitled event"}" fell ${daysFromToday} days away, outside the ${maxDays}-day filter.`);
            }

            validatedDateChecks += 1;
        }
    ...
    async assertUpcomingEventEmptyState() {
        const visibleCount = await this.countVisibleUpcomingEvents();
        const emptyStateVisible = await this.hasUpcomingEventEmptyState();

        if (visibleCount > 0) {
            throw new Error(`Expected no dashboard results, but found ${visibleCount} visible event cards.`);
        }

        if (!emptyStateVisible) {
            throw new Error("Expected the dashboard empty-state banner, but it was not visible.");
        }
    }

    async applyDashboardFilter(filterDefinition) {
        if (!filterDefinition) {
            return;
        }

        if (filterDefinition.kind !== "upcoming-date") {
            throw new Error(`Unsupported dashboard filter kind: ${filterDefinition.kind}`);
        }

        await this.selectOptionByText(filterDefinition.label);
    }

    async resetUpcomingEventFilter(defaultLabel = "Within 3 months") {
        const resetButton = await $('//button[contains(normalize-space(.), "Reset") or contains(normalize-space(.), "Clear")]');

        if (await resetButton.isExisting().catch(() => false)) {
            try {
                await resetButton.waitForClickable({ timeout: 3000 });
                await resetButton.click();
                await this.waitForDashboardResultsToSettle();
            } catch (error) {
                await browser.execute((el) => el.click(), resetButton);
                await this.waitForDashboardResultsToSettle();
            }
        }

        const currentFilterText = await this.getFilterText();

        if (!currentFilterText || !currentFilterText.includes(defaultLabel)) {
            await this.selectOptionByText(defaultLabel);
        }
    }

    async runDashboardFilterScenario(scenario, { baselineCount } = {}) {
        const allFilters = scenario.filters || [];

        for (const oneFilter of allFilters) {
            await this.applyDashboardFilter(oneFilter);
        }

        for (const oneFilter of allFilters) {
            if (oneFilter.kind === "upcoming-date" && Number.isFinite(oneFilter.maxDays)) {
                await this.assertVisibleEventsWithinDays(oneFilter.maxDays, {
                    allowNoResults: Boolean(scenario.allowNoResults)
                });
            }
        }

        if (scenario.expectNoResultsWhenEmpty) {
            const currentVisibleCount = await this.countVisibleUpcomingEvents();

            if (currentVisibleCount === 0) {
                await this.assertUpcomingEventEmptyState();
            }
        }

        if (scenario.expectVisibleResults) {
            const currentVisibleCount = await this.countVisibleUpcomingEvents();

            if (currentVisibleCount === 0) {
                throw new Error(`Scenario "${scenario.name}" expected visible dashboard results, but none were shown.`);
            }

            if (Number.isFinite(baselineCount) && baselineCount > 0) {
                if (currentVisibleCount > baselineCount) {
                    throw new Error(`Scenario "${scenario.name}" increased visible results from ${baselineCount} to ${currentVisibleCount}, which should not happen after filtering.`);
                }
            }
        }
    }
...
async areEventsWithinDays(maxDays) {
    ...
    async hoverOnEventCard(oneEventCard) {
        const cardRoot = await this.getEventCardRoot(oneEventCard);
        await this.jsScrollIntoView(cardRoot);
        await cardRoot.moveTo();
        await browser.pause(200);
    }

    async getEventCardRoot(oneEventCard) {
        const cardRoot = await oneEventCard.$('./ancestor::div[.//button[starts-with(@data-testid,"case-event-edit-")] and .//*[contains(@class,"fui-Persona__primaryText")]][1]');

        if (await cardRoot.isExisting()) {
            return cardRoot;
        }
    ...
    async getEditButtonFromEventCard(oneEventCard) {
        ...
        const eventScopedEditButton = await cardRoot.$('button[data-testid^="case-event-edit-"][aria-label="Edit"], button[data-testid*="case-event-edit"][aria-label="Edit"]');

        if (await eventScopedEditButton.isExisting()) {
            return eventScopedEditButton;
        }

        const pathBasedEditButton = await cardRoot.$(`.//button[.//*[name()="path" and @d="${this.editIconPathD}"]]`);

        if (await pathBasedEditButton.isExisting()) {
            return pathBasedEditButton;
        }

        return cardRoot.$('button[aria-label="Edit"], [data-testid*="event-edit"]');
    }
    ...
    async getVisibleEditButtonAfterHover(oneEventCard) {
        ...
        await browser.waitUntil(async () => {
            ...
            timeout: 3000,
            interval: 200,
            timeoutMsg: "No event-scoped edit button appeared after hover."
        }).catch(() => false);
    ...
    async selectFirstSelectableDateFromPicker() {
        ...
        try {
            await this.eventDatePicker.waitForClickable({ timeout: 3000 });
            await this.eventDatePicker.click();
        } catch (error) {
            await browser.execute((el) => el.click(), await this.eventDatePicker);
        }

        const selectableDateButtons = await $$('//button[@role="gridcell" and not(@disabled) and (@aria-disabled="false" or not(@aria-disabled))]');

        if (!selectableDateButtons.length) {
            console.warn("Date picker opened, but no selectable date button was available. Keeping existing date.");
            return false;
        }

        const firstDateButton = selectableDateButtons[0];

        try {
            await firstDateButton.waitForClickable({ timeout: 3000 });
            await firstDateButton.click();
        } catch (error) {
            await browser.execute((el) => el.click(), firstDateButton);
        }

        // Close any lingering date picker popover so it does not intercept later field clicks.
        await browser.keys("Escape");
        await browser.pause(150);
    ...
    async closeEventAndWaitForDashboard() {
        await this.ensureEventEditorClosed();
    }
    ...
    async runBeginnerHoverEditSaveAndVerifyFlow() {
        ...
        await this.hoverOnEventCard(reopenTarget.event);
        try {
            await reopenTarget.editButton.waitForClickable({ timeout: 5000 });
            await reopenTarget.editButton.click();
        } catch (error) {
            await browser.execute((el) => el.click(), reopenTarget.editButton);
        }
        await this.waitForEventEditor();
    ...
    async selectDateMonthAndDay(monthName, dayNumber) {
        ...
        const directDayButton = await $(`//button[@role="gridcell" and not(@aria-disabled="true") and (.//span[normalize-space()="${dayNumber}"] or normalize-space(.)="${dayNumber}")]`);

        if (await directDayButton.isDisplayed().catch(() => false)) {
            try {
                await directDayButton.waitForClickable({ timeout: 3000 });
                await directDayButton.click();
            } catch (error) {
                await browser.execute((el) => el.click(), directDayButton);
            }

            return;
        }

        const monthButton = await $(`button[role="gridcell"][aria-label="${monthName}"], button[role="gridcell"][aria-label*="${monthName}"]`);

        if (await monthButton.isDisplayed().catch(() => false)) {
            try {
                await monthButton.waitForClickable({ timeout: 3000 });
                await monthButton.click();
            } catch (error) {
                await browser.execute((el) => el.click(), monthButton);
            }
        }

        const dayButton = await $(`//button[@role="gridcell" and not(@aria-disabled="true") and (.//span[normalize-space()="${dayNumber}"] or normalize-space(.)="${dayNumber}")] | //button[not(@aria-disabled="true") and normalize-space(.)="${dayNumber}"]`);
        await dayButton.waitForDisplayed({ timeout: 5000 });

        try {
            await dayButton.waitForClickable({ timeout: 5000 });
            await dayButton.click();
        } catch (error) {
            await browser.execute((el) => el.click(), dayButton);
        }
    }
    ...
    async editAllUpcomingEventsTheSimpleWay() {
        ...
        for (let i = 0; i < allEvents.length; i++) {
            ...
            if (editButton) {
                ...
                const notesOpened = await this.openNotesPanel().catch(() => false);

                if (!notesOpened) {
                    console.warn("Notes panel did not open for this event, so I skipped this one.");
                    ...
                    } else if (await this.cancelEventButton.isExisting()) {
                        try {
                            await this.cancelEventButton.waitForClickable({ timeout: 3000 });
                            await this.cancelEventButton.click();
                        } catch (error) {
                            await browser.execute((el) => el.click(), await this.cancelEventButton);
                        }
                    }
    ...
    async runRequestedCaseTitleFlow(titleToFind, updatedEventText, updatedDescriptionText, noteText) {
        ...
        await browser.pause(600);
        await this.maybeVisualPause(`Hovered ${caseTitleLabel}`);
        ...
        await this.waitForEventEditor();
        await this.maybeVisualPause(`Opened editor for ${caseTitleLabel}`);
        ...
        await this.eventNoteInput.clearValue();
        await this.eventNoteInput.setValue(noteText);
        await this.maybeVisualPause(`Typed note for ${caseTitleLabel}`);
        ...
        await this.clickSaveEventAndWaitForDashboard();
        await this.waitForDashboard();
        await this.maybeVisualPause(`Saved ${caseTitleLabel}`);
        ...
        await this.waitForEventEditor();
        await this.openNotesPanel();
        await this.maybeVisualPause(`Reopened ${caseTitleLabel} notes`);
        ...
        await this.waitForDashboard();
        await this.maybeVisualPause(`Closed ${caseTitleLabel} editor`);
        await this.ensureEventEditorClosed();
        return savedNoteWasVisible;
    }
