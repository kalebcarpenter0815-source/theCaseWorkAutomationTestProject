...
// New case names not already covered by CASE_SEARCH_TERMS above.
// "Jerry" appears many times in the app; included once — the search loop exercises the term, not the count.
const CASE_SEARCH_TERMS_EXTENDED = [
    "HARDER Father - DO NOT DELETE THIS CASE!",
...
describe("Dashboard My Cases Tests", () => {
    ...
    describe("My Cases Section Display", () => {
        it("should be on app.thecasework dashboard and keep Dashboard selected in sidebar", async () => {
            const currentUrl = await browser.getUrl();
            expect(currentUrl.startsWith("https://app.thecasework.com/")).toBe(true);

            await dashboardMyCasesPage.clickDashboardSidebarButton();
            await dashboardPage.waitForDashboard();

            const dashboardSelected = await dashboardMyCasesPage.isDashboardSidebarSelected();
            expect(dashboardSelected).toBe(true);
        });
    ...
    describe("Search Functionality", () => {
        it("should click the search info button, wait, and click it again", async () => {
        ...
        it("should search for all provided case names", async () => {
            for (const caseName of CASE_SEARCH_TERMS) {
                ...
                await dashboardMyCasesPage.clearSearchInputAfterEntry();
            }
        });

        it("should handle random gibberish search - should return no results", async () => {
            await dashboardMyCasesPage.searchCases("dlkfdsjdjfkfl12345678910");
            await dashboardMyCasesPage.waitForCaseTable();
            const emptyVisible = await dashboardMyCasesPage.isEmptyStateVisible();
            expect(emptyVisible).toBe(true);

            await expect(dashboardMyCasesPage.emptyStateMessage).toHaveText(expect.stringContaining("You have no cases"));
            await dashboardMyCasesPage.clearSearchInputAfterEntry();
            await dashboardMyCasesPage.waitForCaseTable();

            const tableVisible = await dashboardMyCasesPage.isCaseTableVisible();
            const emptyVisibleAfterClear = await dashboardMyCasesPage.isEmptyStateVisible();
            expect(tableVisible || emptyVisibleAfterClear).toBe(true);
        });
    });
    ...
    describe("Search Functionality - Extended Cases", () => {
        it("should search for all extended case names", async () => {
            for (const caseName of CASE_SEARCH_TERMS_EXTENDED) {
                await dashboardMyCasesPage.searchCases(caseName);
                await dashboardMyCasesPage.waitForCaseTable();
                const tableVisible = await dashboardMyCasesPage.isCaseTableVisible();
                const emptyVisible = await dashboardMyCasesPage.isEmptyStateVisible();
        ...
        it("should handle random gibberish search in extended suite - should return no results", async () => {
            await dashboardMyCasesPage.searchCases("zzzzzzzzzzzzzzz99999999999");
            await dashboardMyCasesPage.waitForCaseTable();
            const emptyVisible = await dashboardMyCasesPage.isEmptyStateVisible();
            expect(emptyVisible).toBe(true);
