...
describe("Dashboard Templates Card Tests", () => {
    ...
    async function runAllEditChunksForTemplate(templateName) {
        ...
        await dashboardTemplatesPage.openEditFormForTemplate(templateName);

        // Chunk 1 fields
        await dashboardTemplatesPage.clearAndType(dashboardTemplatesPage.templateNameInput, templateName);
        await dashboardTemplatesPage.clearAndType(dashboardTemplatesPage.templateDescriptionInput, "Pokemon thingsss");

        // Chunk 2 actions
        await dashboardTemplatesPage.clickVisibleInfoButtons();
        await dashboardTemplatesPage.completeAllowedStatusesChunk();

        // Chunk 3 blocks
        await dashboardTemplatesPage.clearAndType(
            dashboardTemplatesPage.shortDescriptionFieldNameInput,
            "Description / Action / Proceeding"
        );
        await dashboardTemplatesPage.clearAndType(
            dashboardTemplatesPage.shortDescriptionDefaultTextInput,
            "Pikachu and his friends had fun today!"
        );
        await dashboardTemplatesPage.clearAndType(
            dashboardTemplatesPage.overviewInput,
            "Default text"
        );
        await dashboardTemplatesPage.clearAndType(
            dashboardTemplatesPage.initialNoteInput,
            "fsjdksjsdkfdklsjalskdf912342321331"
        );
        await dashboardTemplatesPage.selectEngagementTemplateByName("Engagement? To whom?");

        // Chunk 4 actions
        await dashboardTemplatesPage.completeMilestoneEventTaskChunk();
    ...
    it("should run step-9 chunk 2 for Pikachu info buttons and statuses", async () => {
        await openCaseTemplatesCard();

        // Open Edit on Pikachu.
        await dashboardTemplatesPage.openEditFormForTemplate("Pikachu");

        // Click visible info buttons and complete Allowed Statuses section.
        await dashboardTemplatesPage.clickVisibleInfoButtons();
        await dashboardTemplatesPage.completeAllowedStatusesChunk();
    ...
    it("should run step-9 chunk 3 for Pikachu text blocks and engagement template", async () => {
        await openCaseTemplatesCard();

        // Open Edit on Pikachu.
        await dashboardTemplatesPage.openEditFormForTemplate("Pikachu");

        // Description and Overview block.
        await dashboardTemplatesPage.clearAndType(
            dashboardTemplatesPage.shortDescriptionFieldNameInput,
            "Description / Action / Proceeding"
        );
    ...
    it("should run step-9 chunk 4 for Pikachu milestone event and task", async () => {
        await openCaseTemplatesCard();

        // Open Edit on Pikachu.
        await dashboardTemplatesPage.openEditFormForTemplate("Pikachu");

        // Create/Edit Milestone section: milestone, event, and task flow.
        await dashboardTemplatesPage.completeMilestoneEventTaskChunk();
    ...
    it("should run step-9 all chunks for You're the Best Around", async () => {
        await runAllEditChunksForTemplate("You're the Best Around");
    });

    it("should run step-9 all chunks for You're the Best Around-Copy", async () => {
