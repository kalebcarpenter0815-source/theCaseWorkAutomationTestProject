...
describe('Dashboard Add Task Modal Tests', () => {
     ...
     */
    it('should complete Add Task form for each Assign To person with all Case selections', async function () {
        ...
        for (const assignToPerson of assignToPersons) {
            ...
            let completedCasesForAssignee = 0;

            // Loop through each Case
            for (const caseItem of cases) {
                if (completedCasesForAssignee >= 1) {
                    break;
                }
                ...
                // ===== Step 2: Select Case =====
                const caseSelected = await dashboardAddTaskModal.selectCaseOption(caseItem.label);
                if (!caseSelected) {
                    console.warn(`Skipping unavailable Add Task case option: ${caseItem.label}`);
                    await dashboardAddTaskModal.cancelForm();
                    continue;
                }

                // ===== Step 3: Select Assign To Person =====
                const assignToSelected = await dashboardAddTaskModal.selectAssignToOption(assignToPerson.name);
                if (!assignToSelected) {
                    console.warn(`Skipping unavailable Assign To option: ${assignToPerson.name}`);
                    await dashboardAddTaskModal.cancelForm();
                    continue;
                }

                // ===== Step 4: Select Milestone (#1 - New Milestone) =====
                const milestoneSelected = await dashboardAddTaskModal.selectMilestoneOption('#1 - New Milestone');
                if (!milestoneSelected) {
                    console.warn('Skipping case because milestone option is unavailable: #1 - New Milestone');
                    await dashboardAddTaskModal.cancelForm();
                    continue;
                }
