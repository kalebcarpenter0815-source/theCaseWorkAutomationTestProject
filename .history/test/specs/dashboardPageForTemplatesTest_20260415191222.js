...
describe("Dashboard Templates Card Tests", () => {
    ...
    async function runTemplateEditFlowFromBestAroundToEndWithRelogin() {
        await openCaseTemplatesCard();
        await ensureTemplateExists("You're the Best Around-Copy");

        const allNames = await dashboardTemplatesPage.getAllTemplateNames();
        const uniqueNames = [...new Set(allNames)];
        const startName = "You're the Best Around";
        const startIndex = uniqueNames.indexOf(startName);
        const targetNames = startIndex >= 0 ? uniqueNames.slice(startIndex) : [
            "You're the Best Around",
            "You're the Best Around-Copy",
        ];

        for (let i = 0; i < targetNames.length; i += 1) {
            const templateName = targetNames[i];
            await runAllEditChunksForTemplate(templateName);

            // Match requested flow: sign out/in between template runs.
    ...
    it("chunked part 4b should sign out/in and run from You're the Best Around through final row", async () => {
        await runTemplateEditFlowFromBestAroundToEndWithRelogin();
    });
