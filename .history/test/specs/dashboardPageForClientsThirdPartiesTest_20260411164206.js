...
async function runOneClient(clientName, occurrence, index) {
    ...
    for (let phoneIndex = 0; phoneIndex < data.phones.length; phoneIndex += 1) {
        if (phoneIndex === 1) {
            await dashboardClientsThirdPartiesPage.verifyAddContactDialogReadyForPhoneNumbers();
        }
    ...
    const addContactDialogStillOpen = await dashboardClientsThirdPartiesPage.addContactHeading.isDisplayed().catch(() => false);
    if (addContactDialogStillOpen) {
        await dashboardClientsThirdPartiesPage.cancelAddContactDialog();
    }
