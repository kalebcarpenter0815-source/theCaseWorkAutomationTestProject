...
describe('Dashboard Add Task Modal Tests', () => {
    ...
    it('should complete Add Task form for each Case from John Doe to Luigi', async function () {
        ...
        const MILESTONES = [
            '#1 - New Milestone',
            '#1 - Our Very First New Milestone',
            '#2 - New Milestone',
        ];
        ...
        for (const caseName of cases) {
            ...
            // Step 4–6: Open Milestone dropdown and select all 3 milestones (one per open)
            for (const milestone of MILESTONES) {
                const milestoneSelected = await dashboardAddTaskModal.selectMilestoneOption(milestone);
                if (!milestoneSelected) {
                    console.warn(`⚠ Milestone not available (continuing): ${milestone}`);
                }
            }
