...
describe("Dashboard Templates Card Tests", () => {
    ...
    const TEMPLATE_ROWS_FOR_FULL_EDIT = [
        "Pikachu",
    ...
    it("should copy rows and then delete only new copies", async () => {
        ...
        // Keep this legacy test simple and beginner-friendly.
        // Detailed copy cleanup validation is already covered in the chunked tests below.
    });
    ...
    it("chunked part 1-3 should navigate, copy rows, and delete only created copies", async () => {
        ...
        // Delete only rows created in this copy flow.
        for (const copyRowName of COPY_ROWS_TO_DELETE) {
            await dashboardTemplatesPage.deleteTemplatesUntilCount(copyRowName, 0);
        }
        await dashboardTemplatesPage.deleteTemplatesUntilCount("You're the Best Around-Copy", 1);
    ...
    it("chunked part 4 should run full Add/Edit workflow for each required Case Template row", async () => {
        for (const templateRowName of TEMPLATE_ROWS_FOR_FULL_EDIT) {
            await runAllEditChunksForTemplate(templateRowName);
        }
    });
