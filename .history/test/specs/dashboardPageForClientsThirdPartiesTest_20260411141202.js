...
describe("Dashboard Clients / 3rd Parties Tests", () => {
    ...
    describe("Row Hover and Three Dot Menu", () => {
        ...
        it("should expose row action controls", async () => {
            const actionButtons = await dashboardClientsThirdPartiesPage.rowActionButtons;
            expect(actionButtons.length).toBeGreaterThan(0);
        });
    ...
    describe("Edit Client Page Flow", () => {
        ...
        it("should display either breadcrumb navigation or Cancel button", async () => {
            const breadcrumbVisible = await dashboardClientsThirdPartiesPage.breadcrumbClientsLink.isDisplayed().catch(() => false);
            const cancelVisible = await dashboardClientsThirdPartiesPage.cancelEditButton.isDisplayed().catch(() => false);
            expect(breadcrumbVisible || cancelVisible).toBe(true);
        });
        ...
        it("should open the Add Phone Number dialog", async () => {
            await dashboardClientsThirdPartiesPage.openAddPhoneNumberDialog();
            await expect(dashboardClientsThirdPartiesPage.addPhoneNumberDialogHeading).toBeDisplayed();
        });

        it("should display fields in the Add Phone Number dialog", async () => {
            await expect(dashboardClientsThirdPartiesPage.addPhoneNumberInput).toBeDisplayed();
            await expect(dashboardClientsThirdPartiesPage.phoneTypeDropdown).toBeDisplayed();
            await expect(dashboardClientsThirdPartiesPage.primaryPhoneToggle).toBeDisplayed();
        });

        it("should show at least one phone type option", async () => {
            await dashboardClientsThirdPartiesPage.phoneTypeDropdown.click();
            await expect(dashboardClientsThirdPartiesPage.phoneTypeOption("Cell")).toBeDisplayed();
            await browser.keys("Escape");
        });

        it("should close the Add Phone Number dialog", async () => {
