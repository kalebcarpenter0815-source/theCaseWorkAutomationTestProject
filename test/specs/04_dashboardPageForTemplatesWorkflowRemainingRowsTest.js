...
describe("Case Templates Workflow Remaining Rows", () => {
    ...
    it("edits the You're the Best Around row", async () => {
        const secondaryExists = await dashboardTemplatesWorkflowPage.isTemplatePresent(targets.secondaryName);
        if (!secondaryExists) {
            return;
        }
    ...
    it("edits the You're the Best Around-Copy row", async () => {
        const copyExists = await dashboardTemplatesWorkflowPage.isTemplatePresent(targets.secondaryCopyName);
        if (!copyExists) {
            return;
        }
