...
class DashboardTemplatesPage extends Page {
    infoIconByLabel(label) {
        return $(`(//*[self::span or self::label or self::em][contains(normalize-space(),"${label}")]/ancestor::*[1]//button[contains(@id,"__infoButton")] | //*[self::span or self::label or self::em][contains(normalize-space(),"${label}")]/following::button[contains(@id,"__infoButton")][1])[1]`);
    }

    caseTypeOptionByText(optionText) {
        return $(`(
            //button[normalize-space()="${optionText}"] |
            //button[.//*[contains(normalize-space(),"${optionText}")]] |
            //*[@role="option" and contains(normalize-space(),"${optionText}")] |
            //input[@type="checkbox" and (@value="${optionText}" or @id="${optionText}")] |
            //label[contains(normalize-space(),"${optionText}")]
        )[1]`);
    }

    statusOptionByText(optionText) {
        return $(`(
            //input[contains(@id,":${optionText}")] |
            //label[contains(normalize-space(),"${optionText}")] |
            //*[@role="option" and contains(normalize-space(),"${optionText}")] |
            //button[contains(normalize-space(),"${optionText}")]
        )[1]`);
    ...
    async clickStatusOptionIfPresent(optionText) {
        const option = this.statusOptionByText(optionText);
        if (await option.isExisting()) {
            await option.click();
        }
    }

    async selectCaseTypeOption(optionText) {
        const option = this.caseTypeOptionByText(optionText);
        if (await option.isExisting()) {
            await option.click();
            return;
        }
        ...
        await this.caseTypeDropdownMenuButton.clearValue();
        await this.caseTypeDropdownMenuButton.setValue(optionText);
        await browser.keys("Enter");
    }

    async clickInfoPopoverClickHere() {
        const clickHereText = await $('(//*[contains(normalize-space(), "Click here")])[last()]');
        await clickHereText.waitForDisplayed({ timeout: 10000 });

        const clickableParent = await clickHereText.$('./ancestor::button[1] | ./ancestor::a[1]');
        if (await clickableParent.isExisting()) {
            await clickableParent.click();
            return;
        }

        await clickHereText.click();
    }

    async getVisibleInfoPopoverText() {
        const popoverText = await $('(//div[@role="tooltip" or @role="dialog" or contains(@class, "Popover") or contains(@class, "Teaching")])[last()]//*[self::span or self::p][string-length(normalize-space()) > 0]');
        await popoverText.waitForDisplayed({ timeout: 10000 });
        return await popoverText.getText();
    }
    ...
    async clickAllowedStatusesClickHereButton() {
        await this.clickInfoPopoverClickHere();
    }
    ...
    get caseTypeDropdownMenuButton() {
        return $('//input[@data-testid="case-type-combobox"] | //label[contains(normalize-space(), "Case Type")]/following::input[1]');
    }
    ...
    get bigZestyShreckDropdownMenuButton() {
        return this.caseTypeOptionByText("big zesty shrek");
    }

    async clickBigZestyShreckDropdownMenuButton() {
        await this.selectCaseTypeOption("big zesty shrek");
    }

    get CaseType_1777908064990DropdownMenuButton() {
        return this.caseTypeOptionByText("CaseType_1777908064990");
    }

    async clickCaseType_1777908064990DropdownMenuButton() {
        await this.selectCaseTypeOption("CaseType_1777908064990");
    }

    get CaseType_1777908679399DropdownMenuButton() {
        return this.caseTypeOptionByText("CaseType_1777908679399");
    }

    async clickCaseType_1777908679399DropdownMenuButton() {
        await this.selectCaseTypeOption("CaseType_1777908679399");
    }

    get CaseType_1777909408167DropdownMenuButton() {
        return this.caseTypeOptionByText("CaseType_1777909408167");
    }

    async clickCaseType_1777909408167DropdownMenuButton() {
        await this.selectCaseTypeOption("CaseType_1777909408167");
    }

    get regularDropdownMenuButton() {
        return this.caseTypeOptionByText("Regular");
    }

    async clickRegularDropdownMenuButton() {
        await this.selectCaseTypeOption("Regular");
    }
    ...
    async clickNewDropdownMenuButtonSelectionNew() {
        await this.clickStatusOptionIfPresent("new");
    }
    ...
    // ================================================
    // Info Icon Buttons in all the pages listed here! 
    // ================================================

    get caseTemplatesInfoIconBtn() {
        return $('//span[contains(normalize-space(), "Case Templates")]/ancestor::span[contains(@class, "fui-InfoLabel")]//button[contains(@id, "__infoButton")]');
    }

    async clickCaseTemplatesInfoIconBtn() {
        await this.caseTemplatesInfoIconBtn.click();
        expect(await this.caseTemplatesInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.caseTemplatesInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getCaseTemplatesInfoIconText() {
        const infoText = await $('//span[contains(text(), "Easily create new cases with predefined types and data using Case Templates.")]');
        return await infoText.getText();
    }
    
    get caseTypeInfoIconBtn() {
        return this.infoIconByLabel("Case Type");
    }

    async clickCaseTypeInfoIconBtn() {
        await this.caseTypeInfoIconBtn.click();
        expect(await this.caseTypeInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.caseTypeInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getCaseTypeInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }


    get allowedStatusesInfoIconBtn() {
        return this.infoIconByLabel("Allowed Statuses");
    }

    async clickAllowedStatusesInfoIconBtn() {
        await this.allowedStatusesInfoIconBtn.click();
        expect(await this.allowedStatusesInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.allowedStatusesInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getAllowedStatusesInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }

    get allowedStatusesInfoIconBtnClickHereButton() {
        return $('//span[contains(text(), "Click here")]');
    }

    async clickAllowedStatusesInfoIconBtnClickHereButton() {
        await this.clickInfoPopoverClickHere();
    }

    async getAllowedStatusesInfoIconBtnClickHereButtonText() {
        const infoText = await $('//span[contains(text(), "Click here")]');
        return await infoText.getText();
    }

    get descriptionAndOverviewInfoIconBtn() {
        return this.infoIconByLabel("Description and Overview");
    }

    async clickDescriptionAndOverviewInfoIconBtn() {
        await this.descriptionAndOverviewInfoIconBtn.click();
        expect(await this.descriptionAndOverviewInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.descriptionAndOverviewInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getDescriptionAndOverviewInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }

    get descriptionAndOverviewShortDescriptionField() {
        return this.infoIconByLabel("Short Description");
    }

    async clickDescriptionAndOverviewShortDescriptionField() {
        await this.descriptionAndOverviewShortDescriptionField.click();
        expect(await this.descriptionAndOverviewShortDescriptionField.isDisplayed()).toBeTruthy();
        expect(await this.descriptionAndOverviewShortDescriptionField.isEnabled()).toBeTruthy();
    }

    async getDescriptionAndOverviewShortDescriptionFieldText() {
        return await this.getVisibleInfoPopoverText();
    }

    get descriptionAndOverviewOverviewFieldInfoIconBtn() {
        return this.infoIconByLabel("Overview");
    }

    async clickDescriptionAndOverviewOverviewFieldInfoIconBtn() {
        await this.descriptionAndOverviewOverviewFieldInfoIconBtn.click();
        expect(await this.descriptionAndOverviewOverviewFieldInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.descriptionAndOverviewOverviewFieldInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getDescriptionAndOverviewOverviewFieldInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }

    get initialNoteInfoIconBtn() {
        return this.infoIconByLabel("Initial Note");
    }

    async clickInitialNoteInfoIconBtn() {
        await this.initialNoteInfoIconBtn.click();
        expect(await this.initialNoteInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.initialNoteInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getInitialNoteInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }

    get engagementTemplateInfoIconBtn() {
        return this.infoIconByLabel("Engagement Template");
    }

    async clickEngagementTemplateInfoIconBtn() {
        await this.engagementTemplateInfoIconBtn.click();
        expect(await this.engagementTemplateInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.engagementTemplateInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getEngagementTemplateInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }

    get engagementTemplateInfoIconBtnClickHereButton() {
        return $('//span[text()="Click here"]');
    }

    async clickEngagementTemplateInfoIconBtnClickHereButton() {
        await this.clickInfoPopoverClickHere();
    }

    async getEngagementTemplateInfoIconBtnClickHereButtonText() {
        const infoText = await this.engagementTemplateInfoIconBtnClickHereButton;
        return await infoText.getText();
    }

    get milestoneInfoIconBtn() {
        return this.infoIconByLabel("Milestones");
    }

    async clickMilestoneInfoIconBtn() {
        await this.milestoneInfoIconBtn.click();
        expect(await this.milestoneInfoIconBtn.isDisplayed()).toBeTruthy();
        expect(await this.milestoneInfoIconBtn.isEnabled()).toBeTruthy();
    }

    async getMilestoneInfoIconText() {
        return await this.getVisibleInfoPopoverText();
    }

    get eventsInfoIconButton() {
        return this.infoIconByLabel("Events");
    }

    async clickEventsInfoIconButton() {
        await this.eventsInfoIconButton.click();
        expect(await this.eventsInfoIconButton.isDisplayed()).toBeTruthy();
        expect(await this.eventsInfoIconButton.isEnabled()).toBeTruthy();
    }

    async getEventsInfoIconButtonText() {
        return await this.getVisibleInfoPopoverText();
    }
    ...
    async clickActiveEngagementsDropdownMenuButtonSelectionTestifyingSlashDepo() {
        await this.clickStatusOptionIfPresent("Testifying/Depo");
    }
    ...
    async clickCompletedDropdownMenuButtonSelectionCompleted() {    
        await this.clickStatusOptionIfPresent("Completed");
    }
    ...
    async clickClosedDropdownMenuButtonSelectionClosed() {
        await this.clickStatusOptionIfPresent("Closed");
    }
    ...
    async clickRemovedDropdownMenuButtonSelectionRemovedTest() {
        await this.clickStatusOptionIfPresent("Removed");
    }
    ...
    get initalNoteNoteTextboxField() {
        return $('(//label[contains(normalize-space(),"Initial Note")]/following::textarea[1] | //label[contains(normalize-space(),"Initial Note")]/following::input[1] | //textarea[@name="initialNote"] | //textarea[contains(@data-testid,"initial-note")])[1]');
    }
    ...
    get engagementTemplateSelectTemplateDropdown() {
        return $('(//label[contains(normalize-space(),"Select Template")]/following::input[1] | //label[contains(normalize-space(),"Select Template")]/following::button[1] | //input[contains(@data-testid,"select-template")])[1]');
    }
    ...
    async clickSelectTemplateCustomDashCopyButton() {
        if (await this.selectTemplateCustomDashCopyButton.isExisting()) {
    ...
    async clickSelectTemplateNewCopyButton() {
        if (await this.selectTemplateNewCopyButton.isExisting()) {
    ...
    async clickSelectTemplateEngagementToWhomMenuButton() {
        if (await this.selectTemplateEngagementToWhomMenuButton.isExisting()) {
    ...
    async clickSelectTemplateCopyrightDashRetentionAgreementButton() {
        if (await this.selectTemplateCopyrightDashRetentionAgreementButton.isExisting()) {
    ...
    async clickSelectTemplateStandardBillingAndPaymentTermsButton() {
        if (await this.selectTemplateStandardBillingAndPaymentTermsButton.isExisting()) {
    ...
    async clickAddMilestoneButton() {
        // Close any open combobox popover before interacting with the milestone button.
        await browser.keys("Escape");
        if (!(await this.addMilestoneButton.isDisplayed())) {
            await this.addMilestoneButton.waitForDisplayed({ timeout: 10000 });
        }
        if (!(await this.addMilestoneButton.isEnabled())) {
            await this.addMilestoneButton.waitForEnabled({ timeout: 10000 });
        }

        try {
            await this.addMilestoneButton.click();
        } catch {
            await browser.execute((el) => el.click(), await this.addMilestoneButton);
        }
   ...
   get creatSlashEditMilestoneMilestoneNameInputField() {
       return $('(//input[@data-testid="milestone-template-name-input"] | //label[contains(normalize-space(),"Milestone Name")]/following::input[1])[1]');
   }
   ...
   async enterMilestoneName(milestoneName) {
    ...
    const input = await this.creatSlashEditMilestoneMilestoneNameInputField;
   ...
   get createSlashEditMilestoneDescriptionInputField() {
       return $('((//*[contains(normalize-space(),"Create/Edit Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//textarea[@data-testid="milestone-template-description-input"] | (//*[contains(normalize-space(),"Create/Edit Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//label[contains(normalize-space(),"Description")]/following::textarea[1])[1]');
   }


   async enterMilestoneDescription(description) {
       const input = await this.createSlashEditMilestoneDescriptionInputField;
   ...
   get createSlashEditMilestoneDueDaysFromCreatedInputField() {
       return $('(//input[@data-testid="milestone-template-due-days-input"] | //label[contains(normalize-space(),"Due Days")]/following::input[1])[1]');
   }
   ...
   async enterMilestoneDueDaysFromCreated(days) {
    ...
    const input = await this.createSlashEditMilestoneDueDaysFromCreatedInputField;
   ...
   get createSlashEditMilestoneDeliverablesInputField() {
       return $('(//textarea[@data-testid="milestone-template-deliverables-input"] | //label[contains(normalize-space(),"Deliverables")]/following::textarea[1])[1]');
   }
   ...
   async enterMilestoneDeliverables(deliverables) {
    ...
    const input = await this.createSlashEditMilestoneDeliverablesInputField;
   ...
   get createSlashEditMilestoneAddEventButton() {
       return $('(//button[@data-testid="link-button-Add Event"])[1]');
   }


   async clickCreateSlashEditMilestoneAddEventButton() {
       const taskDialog = $('(//div[@role="dialog" and .//*[contains(normalize-space(),"Add/Edit Task in Milestone")]])[last()]');
       if (await taskDialog.isExisting() && await taskDialog.isDisplayed()) {
           await taskDialog.waitForDisplayed({ timeout: 10000, reverse: true });
       }

       await browser.waitUntil(async () => {
           return await browser.execute(() => {
               const visibleBackdrops = Array.from(document.querySelectorAll('.fui-DialogSurface__backdrop')).filter((el) => {
                   const style = window.getComputedStyle(el);
                   return style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0';
               });
               // Create/Edit Milestone keeps one backdrop visible; wait for extra overlays to clear.
               return visibleBackdrops.length <= 1;
           });
       }, { timeout: 10000, timeoutMsg: 'Nested dialog backdrop did not clear before Add Event click' });

       const milestoneDialog = await $('(//*[contains(normalize-space(),"Create/Edit Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"])[last()]');
       await milestoneDialog.waitForDisplayed({ timeout: 10000 });

       const scopedButtons = await milestoneDialog.$$('button[data-testid="link-button-Add Event"]');
       for (const btn of scopedButtons) {
           ...
           try {
               ...
               try {
                   ...
                   // Try the next visible milestone Add Event button candidate.
               }
           }
       }

       const anyAddEventButton = await milestoneDialog.$('button[data-testid="link-button-Add Event"]');
       if (await anyAddEventButton.isExisting() && await anyAddEventButton.isDisplayed()) {
           try {
               await anyAddEventButton.click();
               return;
           } catch {
               await browser.execute((el) => el.click(), anyAddEventButton);
               return;
           }
       }

       throw new Error('No Add Event button was found while Create/Edit Milestone dialog is active.');
   }
   ...
   get addEventCardEventNameInputField() {
       return $('(//input[@data-testid="event-template-name-input"] | //label[contains(normalize-space(),"Event Name")]/following::input[1])[1]');
   }
   ...
   async enterAddEventCardEventName(eventName) {
       ...
       const input = await this.addEventCardEventNameInputField;
   ...
   get daysFromCreatedAddEventCardInputField() {
       return $('(//input[@data-testid="event-template-due-days-input"] | //label[contains(normalize-space(),"Days From Created") or contains(normalize-space(),"Due Days")]/following::input[1])[1]');
   }
   ...
   async enterDaysFromCreatedAddEventCard(days) {
       ...
       const input = await this.daysFromCreatedAddEventCardInputField;
   ...
   get addEventIsDueDateCheckbox() {
       return $('(//input[@data-testid="event-template-is-due-checkbox"] | //input[@type="checkbox" and (contains(@name,"due") or contains(@id,"due"))] | //button[@role="switch" and (contains(@aria-label,"Due") or contains(@name,"Due"))] | //button[@role="checkbox" and (contains(@aria-label,"Due") or contains(@name,"Due"))] | //label[contains(normalize-space(),"Is Due") or contains(normalize-space(),"Has Due")]/preceding::input[@type="checkbox"][1] | //label[contains(normalize-space(),"Is Due") or contains(normalize-space(),"Has Due")]/following::input[@type="checkbox"][1])[1]');
   }


   async clickAddEventIsDueDateCheckbox() {
       const primary = this.addEventIsDueDateCheckbox;
       if (await primary.isExisting() && await primary.isDisplayed() && await primary.isEnabled()) {
           await primary.click();
           return;
       }

       const fallbacks = await $$('button[role="switch"], button[role="checkbox"], input[type="checkbox"]');
       for (const control of fallbacks) {
           if (!(await control.isDisplayed()) || !(await control.isEnabled())) {
               continue;
           }
           try {
               await control.click();
               ...
               // Try next visible due-date control candidate.
           }
       }

       // Some environments do not render a due-date control in this modal.
   }


   get addEventDescriptionInputField() {
       return $('(//textarea[@data-testid="event-template-description-input"] | //label[contains(normalize-space(),"Description")]/following::textarea[1])[1]');
   }
   ...
   async enterAddEventDescription(description) {
       ...
       const input = await this.addEventDescriptionInputField;
   ...
   get addSlashEditTaskInMilestoneTaskToCompleteInputField() {
       return $('((//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//input[@data-testid="task-template-title-input"] | (//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//label[contains(normalize-space(),"Title")]/following::input[1] | (//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//input[@name="title"] | (//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//input[@type="text"])[1]');
   }


   async enterAddSlashEditTaskInMilestoneTaskToComplete(task) {
       const input = await this.addSlashEditTaskInMilestoneTaskToCompleteInputField;
       await input.waitForDisplayed();
       await input.waitForEnabled();
       await expect(input).toBeDisplayed();
       await expect(input).toBeEnabled();
   ...
   get addSlashEditTaskInMilestoneDescriptionInputField() {
       return $('((//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//textarea[@data-testid="task-template-dialog-description"] | (//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//label[contains(normalize-space(),"Description")]/following::textarea[1] | (//*[contains(normalize-space(),"Add/Edit Task in Milestone")]/ancestor::*[contains(@class,"fui-DialogSurface") or @role="dialog"][1])//textarea[@name="description"])[1]');
   }


   async enterAddSlashEditTaskInMilestoneDescription(description) {
       const input = await this.addSlashEditTaskInMilestoneDescriptionInputField;
       await input.waitForDisplayed();
       await input.waitForEnabled();
       await expect(input).toBeDisplayed();
       await expect(input).toBeEnabled();
...
// ================================
   ...
   get addEventPopUpCardEventNameInputField() {
       return $('(//label[contains(normalize-space(),"Event Name")]/following::input[1] | //input[contains(@data-testid,"event-name")])[1]');
   }

   async enterAddEventPopUpCardEventName(eventName) {
       const input = await this.addEventPopUpCardEventNameInputField;
       await input.waitForDisplayed();
       await input.waitForEnabled();
       await expect(input).toBeDisplayed();
       await expect(input).toBeEnabled();
    ...
    get daysFromCreatedAddEventPopUpCardInputField() {
         return $('(//label[contains(normalize-space(),"Days from Created")]/following::input[1] | //input[contains(@data-testid,"days-from-created")])[1]');
   }

   async enterDaysFromCreatedAddEventPopUpCard(days) {
       const input = await this.daysFromCreatedAddEventPopUpCardInputField;
       await input.waitForDisplayed();
       await input.waitForEnabled();
       await expect(input).toBeDisplayed();
       await expect(input).toBeEnabled();
    ...
    get addEventPopUpCardIsDueDateCheckbox() {
         return $('(//input[@type="checkbox" and (contains(@id,"due") or contains(@name,"due"))] | //button[@role="switch" and contains(@aria-label,"Due")] | //label[contains(normalize-space(),"Is Due") or contains(normalize-space(),"Has Due")]/preceding::input[@type="checkbox"][1] | //label[contains(normalize-space(),"Is Due") or contains(normalize-space(),"Has Due")]/following::input[@type="checkbox"][1])[1]');
   }

   async clickAddEventPopUpCardIsDueDateCheckbox() {
       const control = this.addEventPopUpCardIsDueDateCheckbox;
       if (await control.isExisting() && await control.isDisplayed() && await control.isEnabled()) {
           await control.click();
       }
   }

    get addEventPopUpCardDescriptionInputField() {
         return $('(//label[contains(normalize-space(),"Description")]/following::textarea[1] | //textarea[contains(@data-testid,"description")])[1]');
   }

   async enterAddEventPopUpCardDescription(description) {
       const input = await this.addEventPopUpCardDescriptionInputField;
       await input.waitForDisplayed();
       await input.waitForEnabled();
       await expect(input).toBeDisplayed();
       await expect(input).toBeEnabled();
   ...
   get addSlashEditCaseTemplateMainPageSaveButtonAtTopPage(){
    ...
    get caseTemplatesHeader() {
        return $('//*[self::h1 or self::span][contains(normalize-space(), "Case Templates")]');
    }

    get caseDataTypesHeader() {
        return $('//*[self::h1 or self::span][contains(normalize-space(), "Case Data Types")]');
    }

    get caseTemplateSavedToast() {
        return $('//*[contains(normalize-space(), "Case Template Saved")]');
    }

    get caseTemplateSavedToastDismissButton() {
        return $('//button[@aria-label="Dismiss" or @data-testid="snackbar-close" or @data-testid="notification-close-button"]');
    }

    async dismissCaseTemplateSavedToast() {
            if (!(await this.caseTemplateSavedToast.isExisting())) {
                return;
            }

            await this.caseTemplateSavedToast.waitForDisplayed({ timeout: 15000 });
            if (await this.caseTemplateSavedToastDismissButton.isExisting() && await this.caseTemplateSavedToastDismissButton.isDisplayed()) {
                await this.caseTemplateSavedToastDismissButton.click();
            }

            if (await this.caseTemplateSavedToast.isExisting()) {
                await this.caseTemplateSavedToast.waitForDisplayed({ reverse: true, timeout: 10000 });
            }
    }

    templateRowByName(name) {
        return $(`//div[@role="row"][.//*[normalize-space()="${name}"]]`);
    }

    templateRowMenuButtonByName(name) {
        return $(`//div[@role="row"][.//*[normalize-space()="${name}"]]//button[@aria-label="More items"]`);
    }

    async openTemplateRowMenu(name) {
        const menuBtn = await this.templateRowMenuButtonByName(name);
        await menuBtn.waitForDisplayed({ timeout: 20000 });
        await menuBtn.waitForEnabled({ timeout: 20000 });
        await menuBtn.click();
    }

    get templateContextMenuCopyOption() {
        return $('//div[@data-testid="custom-data-table-context-menu-item-Copy"]');
    }

    get templateContextMenuEditOption() {
        return $('//div[@data-testid="custom-data-table-context-menu-item-Edit"]');
    }

    get templateContextMenuDeleteOption() {
        return $('//div[@data-testid="custom-data-table-context-menu-item-Delete"]');
    }

    async clickTemplateContextMenuCopy() {
        await this.templateContextMenuCopyOption.waitForDisplayed({ timeout: 10000 });
        await this.templateContextMenuCopyOption.click();
    }

    async clickTemplateContextMenuEdit() {
        await this.templateContextMenuEditOption.waitForDisplayed({ timeout: 10000 });
        await this.templateContextMenuEditOption.click();
    }

    async clickTemplateContextMenuDelete() {
        await this.templateContextMenuDeleteOption.waitForDisplayed({ timeout: 10000 });
        await this.templateContextMenuDeleteOption.click();
    }

    confirmDialogByTitle(title) {
        return $(`//*[contains(normalize-space(), "${title}")]`);
    }
    ...
    async clickConfirmDialogYes() {
        await this.confirmDialogYesButton.waitForDisplayed({ timeout: 10000 });
        await this.confirmDialogYesButton.click();
    }

    async clickConfirmDialogNo() {
        await this.confirmDialogNoButton.waitForDisplayed({ timeout: 10000 });
        await this.confirmDialogNoButton.click();
    }

    async waitForTemplateRow(name) {
        const row = await this.templateRowByName(name);
        await row.waitForDisplayed({ timeout: 20000 });
    }

    async waitForTemplateRowToDisappear(name) {
        const row = await this.templateRowByName(name);
        await row.waitForDisplayed({ reverse: true, timeout: 20000 });
    }
}
